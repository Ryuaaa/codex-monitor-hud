...
当前 Windows 版使用 **Win32 + C++17** 构建常驻悬浮窗，已经具备主页、Codex、电脑性能三页、九个可配置模块、真实性能采样，以及通过本机官方 `codex app-server` 读取 Codex 额度、订阅、官方 Token 日汇总和最近任务历史的第一版能力。
...
- 九个模块包括四个电脑性能模块，以及 Codex 五小时额度、每周额度、订阅类型、账号 Token 用量、最近任务历史五个模块。
...
- 设置使用版本化白名单格式保存到 `%LOCALAPPDATA%\CodexMonitorHUD\settings.ini`；旧版设置迁移时保留原有四个模块的选择，不会自动把新增 Codex 模块放到主页。
...
- 九模块排序、主页/所属页独立开关、设置版本迁移、损坏回退和窗口回屏；
...
4. 与任务管理器抽样对照整机 CPU、物理内存、提交量、页面文件、Codex/ChatGPT 进程树和内存前五名。
