...
- 窗口最小化、Codex 页或空主页会完全停止采样定时器；重新进入需要性能数据的页面时重建 CPU 基线并立即完整首采，避免把停采时段误算成一个 5 秒样本。
...
CPU 差值、整机归一化、进程树、内存前五名、模块排序、设置序列化/损坏回退和窗口回屏位于无 Windows 头文件依赖的固定 C++ 测试中。装有 CMake 和 C++17 编译器的平台可以运行：
...
6. 切到 Codex 页、在主页隐藏全部卡或最小化后，确认采样定时器停止；恢复性能需求时立即完整首采，CPU 等待一个新基线后再显示差值。
