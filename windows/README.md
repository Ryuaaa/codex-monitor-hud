...
- 设置使用版本化白名单格式保存到 `%LOCALAPPDATA%\CodexMonitorHUD\settings.ini`；当前格式为版本 9。旧版设置迁移时保留用户已有模块选择、五小时/每周额度独立状态和窗口位置，并把旧的独立宽高安全重建为 100% 等比窗口，不会继续保留形变或自动开启新工作。
...
- 额度历史位于 `%LOCALAPPDATA%\CodexMonitorHUD\quota-usage-history.txt`，只保留最近 30 天、最多 10,000 个白名单样本，并以临时文件原子替换。趋势模块在当前页面不可见、窗口隐藏或最小化时停止采样和写入。
...
- 尚未实现性能历史曲线和额度通知；网络速度与磁盘读写实时卡已接入现有 5 秒采样，Token 费用与月度费用预测已作为默认关闭的 Beta 模块加入。
