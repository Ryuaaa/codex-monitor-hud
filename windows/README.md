# Windows 原生版：三页产品外壳里程碑

当前里程碑在 Win32 + C++17 原生悬浮窗中建立了主页、Codex、电脑性能三页，并把已有四张真实性能卡纳入模块注册表和本机设置。它不读取对话、提示词、命令行或账号凭据，也不保存采样历史。

## 已实现

- 默认置顶，可切换置顶状态；支持标准标题栏拖动、连续缩放、最小化、DPI 重排和单实例。
- 顶部可在 `Home`、`Codex`、`Computer` 三页切换；当前页会在下次启动恢复。
- 电脑性能页固定显示已有四张真实性能卡；主页可以从同一模块注册表选择显示哪些卡，并在设置窗口中上移或下移排序。
- Codex 页明确显示“本机 Codex 数据尚未连接”，不生成或模拟账号、额度、任务、Token、服务状态或费用数据。
- 设置保存在当前用户的 `%LOCALAPPDATA%\CodexMonitorHUD\settings.ini`，只包含白名单字段：当前页、主页模块显隐与顺序、窗口位置/大小和置顶状态。写入使用同目录临时文件替换，不写程序目录。
- 启动、显示器布局改变和 DPI 改变时，窗口会约束到现有显示器工作区；损坏或越界的保存值会回退或修正。
- 单一串行后台线程持有性能采样器：每 5 秒请求整机 CPU、物理内存、进程列表及 Codex/ChatGPT 进程树，每 20 秒请求系统提交量、页面文件和全部进程的内存排行；窗口消息线程不再执行系统采样。
- 识别 `Codex.exe`、`ChatGPT.exe` 等已知根进程，并按父进程编号纳入其全部后代。
- 汇总 Codex/ChatGPT 进程树的整机 CPU 份额和工作集内存。
- 显示工作集内存最高的 5 个进程。
- 首次启动和从最小化恢复时立即请求一次后台完整采样；两次 20 秒慢采之间，或某次慢采失败时，保留上次有效的提交量、页面文件和内存排行，不闪成不可用。
- 后台采样不重入：工作线程忙时最多合并一个后续请求，完整慢采优先于快采，正在执行的完整采样也会覆盖同期快采请求。
- 窗口最小化、Codex 页或空主页会停止采样定时器、取消尚未执行的请求并使在途旧结果失效；重新进入需要性能数据的页面时先重建 CPU 基线，再请求完整首采，避免把停采时段误算成一个 5 秒样本。
...
- 系统指标和用户设置目录全部来自 Windows 文档化接口，仅链接 `User32`、`Gdi32`、`Psapi`、`Shell32` 和 `Ole32`；
- x64 和 ARM64 都可由 Visual Studio 的对应工具链编译；
- 5 秒快采只打开 Codex/ChatGPT 进程树；打开全部进程并读取工作集、提交量和页面文件的慢采限制为每 20 秒一次，且都在同一个可停止并回收的后台线程串行执行，避免阻塞窗口消息或为轻量悬浮窗增加不必要负担。
...
CPU 差值、整机归一化、进程树、内存前五名、采样请求合并/失效、模块排序、设置序列化/损坏回退和窗口回屏位于无 Windows 头文件依赖的固定 C++ 测试中。装有 CMake 和 C++17 编译器的平台可以运行：
...
2. 首次启动可在 Home、Codex、Computer 三页切换；电脑页始终显示四张卡，主页只显示设置中勾选的卡且顺序一致。
3. 启动后的第一帧 CPU 显示等待下一次采样，约 5 秒后出现 0–100% 数值。
4. 启动 ChatGPT/Codex 后，根进程与后代数量、CPU 和工作集出现；关闭进程时界面降级或归零，应用不崩溃。
5. 与任务管理器抽样对照整机 CPU、物理内存、提交量、页面文件和内存前五名；确认后三项约 20 秒更新一次，间隔中不闪成不可用。
6. 在完整采样进行时拖动和缩放窗口，界面仍可响应；切到 Codex 页、在主页隐藏全部卡或最小化后，确认定时器停止且旧结果不会回填；恢复性能需求时立即请求完整首采，CPU 等待一个新基线后再显示差值。
7. 在设置中切换卡片显隐、上下移动、关闭置顶，重启后确认全部恢复；Codex 页始终只显示未连接说明，不出现模拟数据。
8. 把窗口移到第二块显示器后退出，拔掉该显示器再启动，窗口应完整回到可见工作区；最小化退出不得保存成不可见状态。
9. 点击 `Unpin` 后普通窗口可以盖住它，再点击 `Pin on top` 恢复置顶。
10. 拖动标题栏可移动；拖动任意边或角可缩放；窄窗单列、宽窗双列，在不同 DPI 显示器上仍可读。
11. 再次启动第二个实例时只提示已有实例，不出现第二个悬浮窗。

## 当前明确未完成

- 尚未在真实 Windows x64、Windows ARM64 或多显示器环境完成编译和运行验收；仓库已有 Windows x64 CI，但本地未推送的提交不会触发远端结果。
- 尚未实现 Codex 账户额度、任务、Token/费用、网络/磁盘、历史曲线、登录启动、更新、安装包或签名。
- 当前设置只覆盖本里程碑要求的主页模块、当前页、窗口和置顶；颜色、透明度、刷新间隔、角落预设及锁定位置/大小仍未移植。
