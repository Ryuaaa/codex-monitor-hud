...
- 每 5 秒采集一次整机 CPU、物理内存、系统提交量、页面文件和进程快照。
- 识别 `Codex.exe`、`ChatGPT.exe` 等已知根进程，并按父进程编号纳入其全部后代。
- 汇总 Codex/ChatGPT 进程树的整机 CPU 份额和工作集内存。
- 显示工作集内存最高的 5 个进程。
- 窗口最小化时停止采样定时器；恢复时重建 CPU 基线，避免把整个最小化时段误算成一个 5 秒样本。
...
- 当前同步采样规模较小，避免为 5 秒一次的轻量快照额外常驻工作线程。
...
4. 与任务管理器抽样对照整机 CPU、物理内存、提交量、页面文件和内存前五名；只要求口径一致和量级合理，不要求瞬时值完全相同。
5. 点击最小化后确认进入任务栏且不再按 5 秒采集；恢复时 CPU 先等待一个新基线，再显示差值。
