...
- 更新助手的严格顺序、全部等待失败不安装、安装失败时仅恢复已验证旧版本、未验证 EXE 不启动、签名后版本身份校验，以及 Windows 上按进程编号、创建时间和程序路径等待精确旧进程。
...
- 已有每日一次和手动更新检查、CI 便携 ZIP、SHA-256，以及经过安装/卸载测试的未签名 MSI；安全下载与连续锁定同一 MSI 完成校验和、发布者签名固定、身份校验和同步安装的事务已完成。受控更新助手核心也已接入：它在窗口和单实例初始化前运行，只接受继承的旧 HUD 进程句柄并核对进程编号、创建时间和程序路径，等待退出后才安装；安装成功后还会连续锁定、复核已安装 EXE 的签名、固定发布者和版本身份，再直接重启。尚未把“下载、复制临时助手、传递继承句柄、退出当前 HUD”接到真实更新按钮，也未配置正式 Windows 发布者证书指纹，因此当前不会对用户开放一键安装；登录后自动启动、Windows 正式代码签名和发布流程也未完成。
