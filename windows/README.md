...
当前 Windows 版使用 **Win32 + C++17** 构建常驻悬浮窗，已经具备主页、Codex、电脑性能三页、十个可配置模块、真实性能采样，以及通过本机官方 `codex app-server` 读取 Codex 额度、订阅、官方 Token 日汇总和最近任务历史的第一版能力。
...
- 十个模块包括“电脑压力、主要瓶颈与 Codex/ChatGPT 影响”诊断卡、四个原始性能明细卡，以及 Codex 五小时额度、每周额度、订阅类型、账号 Token 用量、最近任务历史五个模块。
- 每个模块分别提供“显示在主页”和“显示在所属页面”两个开关；五小时额度与每周额度互不绑定。
- 电脑性能模块默认在主页和电脑页显示；Codex 模块默认不加入主页。Codex 页默认显示五小时额度、每周额度、订阅和最近任务历史，Token 用量默认关闭。
- 设置使用版本化白名单格式保存到 `%LOCALAPPDATA%\CodexMonitorHUD\settings.ini`；当前格式为版本 3。旧版设置迁移时保留用户已有选择，不会自动把新增 Codex 模块放到主页。
...
- 十模块排序、主页/所属页独立开关、设置版本迁移、损坏回退和窗口回屏；
...
7. 切换到不需要数据的页面、隐藏所有相关模块和最小化窗口，确认性能采样与 Codex 子进程停止；恢复后立即取得新数据。
8. 模拟接口失败，确认旧值保留、失败提示出现、没有残留 `codex.exe app-server`；恢复后能重新刷新。
9. 把窗口移到第二块显示器后退出，拔掉该显示器再启动，窗口应完整回到可见工作区。

## 当前明确未完成

- 尚未在真实 Windows x64 上完成视觉/交互验收，也未与真实任务管理器和真实 Codex 账号数据做逐项对照。
- 尚未实现网络速度、磁盘读写、历史曲线与持久化、Token 费用与长期预测历史、OpenAI 服务状态和额度通知；月末 Token 线性趋势已显示。
