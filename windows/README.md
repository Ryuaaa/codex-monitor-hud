...
- 本机会话扫描只查看 `codexHome` 下近 30 天的 `sessions/YYYY/MM/DD/rollout-*.jsonl` 和近期 `archived_sessions/*.jsonl`。每轮最多读取 64 MiB、单行最多保留 512 KiB，增量游标只留在内存；重启后会重新进行一次有界扫描。`.jsonl.zst` 暂不解压，并明确标记覆盖不完整。
