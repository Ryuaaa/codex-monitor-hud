...
- 已有每日一次和手动更新检查、CI 便携 ZIP、SHA-256，以及经过安装/卸载测试的未签名 MSI；安全下载与连续锁定同一 MSI 完成校验和、发布者签名固定、身份校验和同步安装的事务已完成。尚未接入独立更新助手的退出/重启/失败恢复编排，也未完成登录后自动启动、Windows 正式代码签名和发布流程。
