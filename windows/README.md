...
当前 Windows 版使用 **Win32 + C++17** 构建常驻悬浮窗，已经具备主页、Codex、电脑性能三页、十二个可配置模块、真实性能采样，以及通过本机官方 `codex app-server` 读取 Codex 额度、订阅、官方 Token 日汇总和最近任务历史的第一版能力。
...
- 十二个模块包括“电脑压力、主要瓶颈与 Codex/ChatGPT 影响”诊断卡、四个原始性能明细卡，以及 Codex 五小时额度、每周额度、额度趋势预测、订阅类型、账号 Token 用量、最近任务历史和 OpenAI 官方服务状态七个模块。
- 每个模块分别提供“显示在主页”和“显示在所属页面”两个开关；五小时额度与每周额度互不绑定。
- 电脑性能模块默认在主页和电脑页显示；Codex 模块默认不加入主页。新安装时，Codex 页默认显示五小时额度、每周额度、额度趋势、订阅、最近任务历史和 OpenAI 官方服务状态，Token 用量默认关闭。
- 设置使用版本化白名单格式保存到 `%LOCALAPPDATA%\CodexMonitorHUD\settings.ini`；当前格式为版本 5。旧版设置迁移时保留用户已有选择，不会自动开启额度趋势或改写已有页面选择。
...
- 十二模块排序、主页/所属页独立开关、设置版本迁移、损坏回退和窗口回屏；
...
- 尚未实现网络速度、磁盘读写、性能历史曲线、Token 费用与长期费用预测和额度通知；月末 Token 线性趋势、额度趋势与 OpenAI 官方服务状态已显示。
