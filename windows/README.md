...
- 已有每日一次和手动更新检查、CI 便携 ZIP、SHA-256，以及经过安装/卸载测试的未签名 MSI；安全下载与连续锁定同一 MSI 完成校验和、发布者签名固定、身份校验和同步安装的事务已完成。受控更新助手核心也已接入：启动方复制并重新验证当前已安装 HUD，只向临时助手继承精确旧进程句柄；助手在窗口和单实例初始化前核对进程编号、创建时间和程序路径，等待退出后才安装，失败时只恢复已验证旧版本，成功后只重启已验证新版。尚未把“两项资产下载、启动助手、关闭当前 HUD”编排接到真实更新按钮，也未配置正式 Windows 发布者证书指纹，因此当前不会对用户开放一键安装；登录后自动启动、Windows 正式代码签名和发布流程也未完成。
