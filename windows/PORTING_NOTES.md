...
审计基线：macOS 1.0 候选提交 `864ddea`。Windows 第三里程碑只修改 `windows/`，没有改动 `overlay/`、macOS 构建/发布脚本或候选产物。
...
| --- | --- | --- |
| 窗口、卡片、设置、模块排序 | `overlay/CodexMonitorHUD.m`、`overlay/HUDView.m` | AppKit 不可复用；按同一信息架构用 Win32 重建。三页、模块注册表、主页显隐/排序、置顶、最小化、拖动和缩放已有第一版。 |
...
| --- | --- | --- |
| 三页面、模块开关/排序、颜色透明度、角落位置、窄栏、锁定位置与大小 | 已实现三页及主页模块开关/排序；其余可实现 | 模块注册与设置状态有可移植固定测试；仍需真实 DPI、多显示器、键盘和屏幕阅读器测试。颜色、透明度、角落预设及锁定尚未移植。 |
...
| Codex 额度、订阅、官方 Token 汇总、任务列表 | 协议上可移植，当前待核验 | 当前 Codex 页只显示“本机 Codex 数据尚未连接”，禁止模拟数据。接入前提是 Windows 本机存在可执行的 Codex `app-server --stdio`，且相关只读方法返回兼容；必须在登录状态下验证。 |
...
3. 验收三页切换、主页显隐/排序、置顶与窗口持久化，以及拔掉外接显示器后的窗口回屏；当前可移植状态测试不能替代这些真实交互。
4. 单独验证 Windows Codex 本机接口和会话路径；确认兼容后再接入账户、额度、任务和 Token 模块。
5. 最后做登录启动、签名、便携 ZIP/安装包和失败可回滚更新；签名与 SmartScreen 状态分别报告。
