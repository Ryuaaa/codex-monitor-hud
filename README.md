...
应用启动4秒后检查一次更新，此后每12小时检查一次；也可在设置、应用菜单或右键菜单点击“检查更新”。发现新版后可一键下载、校验安全摘要、替换旧版并重启，失败会保留旧版。更新来源固定为本仓库的GitHub Release。

注意：当前公开仓库还没有GitHub Release，所以现有旧版不会自动出现更新提示。首次发布带有更新器的安装包后，使用该版本及后续版本的用户才能收到并一键安装未来更新；只下载源码的用户仍可运行上面的安装脚本更新。
