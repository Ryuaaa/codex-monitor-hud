...
> 当前版本：0.8.2 公开预览修复版。核心功能和自动化测试已就绪；完成72小时稳定性、第二台Mac和开发者签名验证后再发布正式1.0。
...
不想自行构建时，可从[GitHub发布页下载0.8.2应用压缩包](https://github.com/Ryuaaa/codex-monitor-hud/releases/tag/v0.8.2)。
...
发布页提供应用压缩包和SHA-256校验文件。0.8.2使用临时本地签名，尚未通过Apple开发者公证；首次手动下载时macOS可能要求用户确认来源。使用0.8.0及后续版本的用户可在应用内收到并一键安装未来更新；只下载源码的用户仍可运行上面的安装脚本更新。
...
第二条必须返回 `quota_available=true`，否则不能把Codex额度功能判定为可用。第三条用于防止自然日Token统计退化；第四条检查集中设置、位置和大小锁定、隐藏停采，以及拖拽时横向和纵向保持同一缩放倍数；第五条检查更新版本比较、发布信息解析和SHA-256安全摘要；第六条会在已安装登录启动服务的Mac上实测更新后的启动服务交接。
