...
点击顶部标签切换主页、Codex和电脑性能页面。主页内容可逐项加入或移除5小时额度、每周额度、订阅、Token用量、模型专属额度、瓶颈判断、电脑核心状态、Codex占用、内存排行和CPU趋势。顶部减号会把窗口真正最小化到macOS程序栏（Dock），点击程序栏图标即可恢复；图钉按钮可以随时切换置顶与非置顶并保存状态。齿轮或右键菜单还可以进入窄栏模式，并设置分页面内容、强调颜色、背景透明度、刷新频率、四角位置、详细模式、本地趋势记录和登录后自动启动。
