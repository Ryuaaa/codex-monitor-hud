...
> 当前版本：0.8.1 公开预览修复版。核心功能和自动化测试已就绪；完成72小时稳定性、第二台Mac和开发者签名验证后再发布正式1.0。
...
不想自行构建时，可从[GitHub发布页下载0.8.1应用压缩包](https://github.com/Ryuaaa/codex-monitor-hud/releases/tag/v0.8.1)。
...
发布页提供应用压缩包和SHA-256校验文件。0.8.1使用临时本地签名，尚未通过Apple开发者公证；首次手动下载时macOS可能要求用户确认来源。使用0.8.0及后续版本的用户可在应用内收到并一键安装未来更新；只下载源码的用户仍可运行上面的安装脚本更新。
