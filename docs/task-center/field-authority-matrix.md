...
统一规则：正式单任务 Markdown 负责当前任务事实，追加式 JSONL 负责历史，所连接的官方 Codex App Server 负责该实例运行态，本机项目映射配置负责目录对应关系。界面筛选配置只属于可删除的显示偏好，不是任务事实源。
...
| 来源/关联 | Markdown `source_refs/related_ids` | 只读数组 | 对话绑定和关联任务 | `related_ids` 可经预览确认写入；`source_refs` 禁止 | 无法识别的引用保留原值，不猜测 | 拒绝自身、重复、非法编号；仅改当前任务 |
...
| 标签 | Markdown `tags` | 只读数组 | 卡片、详情、搜索和标签筛选 | 可经预览确认写入 | Markdown 为唯一权威；保存筛选只保存标签条件 | 拒绝空值、重复、换行、超长和超量 |
| 父子关系 | 子任务 Markdown `parent_id` | 读取正向值；子任务列表在内存反向推导 | 父任务、子任务 | 只写当前任务的 `parent_id` | 不为维护反向关系而批量改父任务 | 拒绝自身和非法编号；反向关系不落盘 |
| 阻塞关系 | 被阻塞任务 Markdown `blocked_by_ids` | 读取正向值；“正在阻塞谁”在内存反向推导 | 阻塞当前任务、当前任务正在阻塞 | 只写当前任务的 `blocked_by_ids` | 不建立第二份反向事实 | 拒绝自身、重复和非法编号；只改当前任务 |
| 评论/人工活动 | `事件/YYYY-MM.jsonl` 的 `comment_added/manual_activity_added` | 详情按需、逐行、有界读取 | 时间、类型、内容、记录人 | 仅追加事件，不改 Markdown | 预览同时绑定任务哈希和事件文件哈希 | 冲突停止；写后回读；正文 2000 字和记录人 120 字上限 |
| 保存筛选 | 应用配置 `saved-task-filters.json` | 进入管理任务后读取 | 项目、状态、标签、归档、视图 | 可保存/更新/删除 | 不是任务事实；可删除重建 | 原子替换、回读和旧配置恢复；不保存搜索词或任务内容 |
| 搜索索引/已读/统计 | 可重建缓存 | 以内存派生，不落盘 | 搜索与计数 | 不写任务 | 删除缓存后可重建 | 若将来落盘，必须排除正文、隐私字段和密钥 |
| 附件、重复、开始日期、工作树绑定 | 正式结构目前未稳定定义 | 不读取为业务字段 | 显示“需要审核是否扩展正式结构” | 禁止 | 不塞入 SQLite/缓存伪造字段 | 先审核正式结构，再设计写入 |
...
当前实现不改变上述权威位置。所有写入均由任务中心界面直接触发，经过预览、明确确认、期望哈希检查、同目录准备文件、原子替换或追加事件、写后回读；冲突停止，提交后失败恢复原任务与原事件。`updated_at` 当前不作隐含写入，准确变更时间由追加事件记录，避免预览之外的第二个字段变化。
...
| 标签 | Markdown `tags` | `tags_changed` | 拒绝空值、重复、换行、超长和超量 |
| 父任务 | 当前任务 Markdown `parent_id` | `parent_changed` | 拒绝自身/非法编号；子任务反向关系只在内存推导 |
| 阻塞任务 | 当前任务 Markdown `blocked_by_ids` | `blockers_changed` | 拒绝自身、重复、非法编号；“正在阻塞谁”只在内存推导 |
| 关联任务 | Markdown `related_ids` 中正式任务编号 | `relations_changed` | 拒绝自身、重复、非法编号 |
| 评论/人工活动 | 不改 Markdown；向月度 JSONL 追加有界事件 | `comment_added` / `manual_activity_added` | 双哈希冲突检查、明确确认和回读；不覆盖事件文件并发变化 |
| 保存筛选 | 应用配置，不写任务 | 无正式任务事件 | 只保存结构化显示条件，不保存搜索词和任务内容 |
| 归档/恢复 | Markdown `record_status` 的 `archived/current` | `archived/restored` | 不移动或重命名文件，不从看板列推导额外状态 |
| 附件、重复、甘特图、AI、自动派发、工作流、Jira、云协作 | 不支持 | 无 | 界面明确提示“正式结构暂不支持”，不写缓存或私有数据库 |
