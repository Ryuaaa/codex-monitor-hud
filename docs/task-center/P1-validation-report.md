...
P0 契约与 P1 只读 MVP 已在隔离工作树形成。按用户 2026-08-24 确认的验收口径，**macOS P1 已达到当前正式验收标准**；Windows 真实桌面验收暂缓，不再阻塞 macOS。macOS arm64 生产包已完成真实窗口、20 次原生打开—关闭、三轮资源基线、Retina 2.0 倍缩放、最小窗口、强制终止和 WebContent 异常隔离验证。
...
- 重新复核同一二进制的默认窗口与最小窗口截图；结合既有真实交互记录，覆盖看板、列表、项目切换、搜索、状态筛选、详情、正文按需读取、活动时间线、空状态和错误降级。Retina backing scale `2.0`、最大窗口和 760×522 原生最小边界已验证；没有发现文字覆盖、控件重叠或核心布局形变。界面美感与信息密度仅余用户主观确认，不是功能验收缺口。
