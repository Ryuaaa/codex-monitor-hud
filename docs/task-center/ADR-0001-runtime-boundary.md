...
3. P1 阶段没有修改 HUD；后续已按本 ADR 的“未来允许的 HUD 交互”加入由用户点击触发的系统启动入口，不共享运行时或后台连接。
4. 任务中心不启动第二个交互式 Codex、本地 HTTP 服务、CDP 端口、注入器、托盘、LaunchAgent、计划任务或后台服务。只有用户按需读取官方历史时，允许启动一个短期 `codex app-server --stdio` 子进程；请求完成、失败或超时后立即回收。最后一个窗口关闭时应用退出。
5. 共享代码只允许位于 `task-center/src/domain/` 的纯数据契约、状态映射、权限规则和固定样例。未来若 HUD 需要摘要，应复制稳定的小型契约为独立原生实现，不得加载任务中心运行时。
6. 任务中心默认进入官方只读的“Codex 活动”，启动时只调用一次有界 `thread/list`；“管理任务”作为可选数据源，只有用户进入后才检测正式任务目录，缺失时也必须再次点击才创建本地任务库。
