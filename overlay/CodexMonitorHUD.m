...
static NSString *FormatReset(NSTimeInterval timestamp) {
    if (timestamp <= 0) return HUDL(@"恢复时间未返回");
    NSTimeInterval remaining = timestamp - NSDate.date.timeIntervalSince1970;
    if (remaining <= 0) return HUDL(@"即将恢复");
    NSInteger minutes = (NSInteger)ceil(remaining / 60.0);
    if (minutes < 60) return [NSString stringWithFormat:HUDL(@"%ld分钟后恢复"), (long)minutes];
    NSInteger hours = minutes / 60;
    NSInteger rest = minutes % 60;
    if (hours < 48) return rest ? [NSString stringWithFormat:HUDL(@"%ld小时%ld分后恢复"), (long)hours, (long)rest] : [NSString stringWithFormat:HUDL(@"%ld小时后恢复"), (long)hours];
    return [NSString stringWithFormat:HUDL(@"%ld天%ld小时后恢复"), (long)(hours / 24), (long)(hours % 24)];
}
...
static NSString *FormatQuotaWindow(double durationMins) {
    ...
    if (minutes <= 0) return HUDL(@"周期未返回");
    if (minutes == 300) return HUDL(@"5小时");
    if (minutes == 10080) return HUDL(@"每周");
    if (minutes % 1440 == 0) return [NSString stringWithFormat:HUDL(@"%ld天"), (long)(minutes / 1440)];
    if (minutes % 60 == 0) return [NSString stringWithFormat:HUDL(@"%ld小时"), (long)(minutes / 60)];
    return [NSString stringWithFormat:HUDL(@"%ld分钟"), (long)minutes];
}
...
static void HUDApplyQuotaDataState(HUDQuotaCard *card, NSString *state, BOOL available, double remaining, NSTimeInterval resetAt, NSString *reachedType) {
    if ([state isEqualToString:@"previous"] && available) {
        card.windowLabel.stringValue = HUDL(@"上次数据");
        card.resetLabel.stringValue = [NSString stringWithFormat:HUDL(@"本轮未返回 · %@"), FormatReset(resetAt)];
    } else if ([state isEqualToString:@"expired"]) {
        card.windowLabel.stringValue = HUDL(@"已过期");
        card.valueLabel.stringValue = HUDL(@"上次数据已过期");
        ...
        card.resetLabel.stringValue = HUDL(@"等待官方返回新周期");
    } else if (available && reachedType.length > 0 && remaining <= 0.01) {
        card.windowLabel.stringValue = HUDL(@"官方触顶");
        card.valueLabel.textColor = NSColor.systemRedColor;
    }
}

static NSString *FormatUSD(double value) {
    return HUDMoney(value);
}

static NSString *FormatForecastHeadline(NSString *headline) {
    if ([headline isEqualToString:HUDL(@"可撑到重置")]) return HUDL(@"可撑到重置");
    if ([headline isEqualToString:HUDL(@"可能提前用完")]) return HUDL(@"可能提前用完");
    if ([headline isEqualToString:HUDL(@"近期用量平稳")]) return HUDL(@"近期平稳");
    return headline.length > 0 ? headline : HUDL(@"积累中");
}

static NSString *FormatDuration(NSInteger seconds) {
    if (seconds < 60) return [NSString stringWithFormat:HUDL(@"%ld秒"), (long)MAX(0, seconds)];
    NSInteger minutes = seconds / 60;
    if (minutes < 60) return [NSString stringWithFormat:HUDL(@"%ld分钟"), (long)minutes];
    NSInteger hours = minutes / 60;
    NSInteger rest = minutes % 60;
    return rest ? [NSString stringWithFormat:HUDL(@"%ld小时%ld分"), (long)hours, (long)rest] : [NSString stringWithFormat:HUDL(@"%ld小时"), (long)hours];
}
...
static NSString *FormatPlan(NSString *plan) {
    ...
    return names[plan ?: @""] ?: HUDL(@"未知订阅");
}

static NSDateFormatter *HUDSubscriptionDateFormatter(NSString *format) {
...
static NSDate *HUDSubscriptionDateFromString(NSString *value) {
    if (![value isKindOfClass:NSString.class] || value.length != 10) return nil;
    NSDateFormatter *formatter = HUDSubscriptionDateFormatter(@"yyyy-MM-dd");
    NSDate *date = [formatter dateFromString:value];
    if (!date || ![[formatter stringFromDate:date] isEqualToString:value]) return nil;
    return [formatter.calendar startOfDayForDate:date];
}

static NSString *HUDSubscriptionDateStringFromDate(NSDate *date) {
    if (![date isKindOfClass:NSDate.class]) return nil;
    return [HUDSubscriptionDateFormatter(@"yyyy-MM-dd") stringFromDate:date];
}

static NSDictionary<NSString *, id> *HUDSubscriptionDatePresentation(NSString *value, NSDate *now) {
    NSDate *date = HUDSubscriptionDateFromString(value);
    if (!date || !now) return nil;
    NSCalendar *calendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    calendar.timeZone = NSTimeZone.localTimeZone;
    NSDate *today = [calendar startOfDayForDate:now];
    NSInteger days = [[calendar components:NSCalendarUnitDay fromDate:today toDate:date options:0] day];
    NSString *relative = days > 0 ? [NSString stringWithFormat:HUDL(@"还剩%ld天"), (long)days] :
                         (days == 0 ? HUDL(@"今天到期/续费") : [NSString stringWithFormat:HUDL(@"已过%ld天 · 请核对"), (long)-days]);
    return @{
        @"fullDate": [HUDSubscriptionDateFormatter(@"yyyy/MM/dd") stringFromDate:date],
        @"shortDate": [HUDSubscriptionDateFormatter(@"M/d") stringFromDate:date],
        @"relative": relative,
        @"days": @(days)
    };
}
...
static NSString *FormatAge(NSTimeInterval timestamp) {
    if (timestamp <= 0) return HUDL(@"连接中");
    NSInteger age = MAX(0, (NSInteger)(NSDate.date.timeIntervalSince1970 - timestamp));
    if (age < 10) return HUDL(@"刚刚");
    if (age < 60) return [NSString stringWithFormat:HUDL(@"%ld秒前"), (long)age];
    return [NSString stringWithFormat:HUDL(@"%ld分钟前"), (long)(age / 60)];
}
...
static NSString *FormatModuleState(NSTimeInterval timestamp, NSString *error, NSTimeInterval maximumAge) {
    if (error.length > 0) return timestamp > 0 ? [NSString stringWithFormat:HUDL(@"失败·上次%@"), FormatAge(timestamp)] : HUDL(@"失败");
    if (HUDTimestampIsStale(timestamp, maximumAge)) return [NSString stringWithFormat:HUDL(@"已过期·上次%@"), FormatAge(timestamp)];
...
- (void)configureApplicationMenu {
    ...
    NSMenuItem *about = [appMenu addItemWithTitle:HUDL(@"关于 Codex Monitor HUD") action:@selector(orderFrontStandardAboutPanel:) keyEquivalent:@""];
    about.target = NSApp;
    [appMenu addItem:NSMenuItem.separatorItem];
    NSMenuItem *show = [appMenu addItemWithTitle:HUDL(@"显示悬浮窗") action:@selector(showHUD:) keyEquivalent:@""];
    show.target = self;
    NSMenuItem *taskCenter = [appMenu addItemWithTitle:HUDL(@"打开任务中心") action:@selector(openTaskCenter:) keyEquivalent:@""];
    taskCenter.target = self;
    NSMenuItem *hide = [appMenu addItemWithTitle:HUDL(@"隐藏 Codex Monitor HUD") action:@selector(hide:) keyEquivalent:@"h"];
    hide.target = NSApp;
    NSMenuItem *checkUpdates = [appMenu addItemWithTitle:HUDL(@"检查更新…") action:@selector(checkForUpdatesManually:) keyEquivalent:@""];
    checkUpdates.target = self;
    [appMenu addItem:NSMenuItem.separatorItem];
    NSMenuItem *quit = [appMenu addItemWithTitle:HUDL(@"退出 Codex Monitor HUD") action:@selector(terminate:) keyEquivalent:@"q"];
    quit.target = NSApp;
...
- (void)presentTaskCenterLaunchFailure:(NSString *)detail {
    ...
    alert.messageText = HUDL(@"暂时无法打开任务中心");
    alert.informativeText = detail.length > 0 ? detail : HUDL(@"尚未找到已安装的 Codex Monitor 任务中心。悬浮窗会继续正常运行。");
    [alert addButtonWithTitle:HUDL(@"打开下载页")];
    [alert addButtonWithTitle:HUDL(@"取消")];
...
- (void)openTaskCenter:(id)sender {
    ...
    [NSWorkspace.sharedWorkspace openApplicationAtURL:applicationURL configuration:configuration completionHandler:^(__unused NSRunningApplication *application, NSError *error) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            NSString *message = [NSString stringWithFormat:HUDL(@"任务中心已找到，但启动失败：%@"), error.localizedDescription ?: HUDL(@"未知错误")];
...
- (void)startCodexProviderIfNeeded {
    ...
    if (![requests isEqualToSet:self.codexProvider.enabledRequestIDs]) self.lastCodexAccountFetchAt = 0;
    self.codexProvider.enabledRequestIDs = requests;
...
- (NSTimeInterval)codexAccountRefreshInterval {
    ...
    BOOL hasError = NO;
    for (NSNumber *requestID in self.codexProvider.enabledRequestIDs) if (snapshot.interfaceFailureKinds[requestID.stringValue]) hasError = YES;
    BOOL quotaLow = [self.codexProvider.enabledRequestIDs containsObject:@2] && ((snapshot.fiveHourAvailable && snapshot.fiveHourRemainingPercent <= 15) || (snapshot.weeklyAvailable && snapshot.weeklyRemainingPercent <= 15));
...
- (void)updateServiceStatusDisplay {
    ...
    NSString *value = snapshot.available ? snapshot.headline : (snapshot.errorText ?: HUDL(@"正在检查官方状态"));
    NSString *detail = snapshot.available ? [NSString stringWithFormat:@"%@ · %@", snapshot.detail ?: HUDL(@"OpenAI整体状态未知"), FormatModuleState(snapshot.updatedAt, snapshot.errorText, 900)] : HUDL(@"未读取账号信息 · 稍后重试");
...
- (NSDictionary *)statusForSnapshot:(NativeSnapshot *)snapshot {
    NSString *title = HUDL(@"未发现明显资源压力"), *code = @"normal";
    ...
    if (snapshot.memoryPressureLevel >= 2) {
        title = codexMemoryDominant ? HUDL(@"内存严重吃紧 · Codex很可能是主因") : HUDL(@"内存严重吃紧 · 主因待判断");
        ...
        title = codexMemoryDominant ? HUDL(@"内存开始吃紧 · Codex可能是主因") : HUDL(@"内存开始吃紧 · 主因待判断");
        code = codexMemoryDominant ? @"memory_codex_warning" : @"memory_warning"; color = NSColor.systemOrangeColor;
    }
    else if (snapshot.thermalLevel >= 2) { title = HUDL(@"系统热压力正在限制性能"); code = @"thermal"; color = NSColor.systemRedColor; }
    else if (snapshot.systemCPUPercent >= 90.0) {
        if (snapshot.codexCPUPercent >= 25.0 && snapshot.codexCPUPercent >= snapshot.systemCPUPercent * 0.4) { title = HUDL(@"CPU吃紧 · Codex很可能是主因"); code = @"cpu_codex"; }
        else { title = HUDL(@"CPU吃紧 · 主要是其他程序"); code = @"cpu_other"; }
        color = NSColor.systemOrangeColor;
    } else if (snapshot.systemCPUPercent >= 75.0) { title = HUDL(@"CPU负载较高"); code = @"cpu_high"; color = NSColor.systemYellowColor; }
...
- (NSString *)topAppsText:(NSArray<NativeTopApp *> *)apps cpu:(BOOL)cpu {
    ...
    return parts.count ? [parts componentsJoinedByString:@" · "] : HUDL(@"暂无");
}
...
- (NSDictionary *)diagnosisForStatusCode:(NSString *)code {
    if ([code hasPrefix:@"memory_codex"]) return @{@"bottleneck": HUDL(@"内存"), @"impact": HUDL(@"高"), @"confidence": HUDL(@"中等置信度 · 推断")};
    if ([code hasPrefix:@"memory_"]) return @{@"bottleneck": HUDL(@"内存"), @"impact": HUDL(@"待判断"), @"confidence": HUDL(@"低置信度")};
    if ([code isEqualToString:@"cpu_codex"]) return @{@"bottleneck": @"CPU", @"impact": HUDL(@"高"), @"confidence": HUDL(@"较高置信度 · 推断")};
    if ([code isEqualToString:@"cpu_other"]) return @{@"bottleneck": @"CPU", @"impact": HUDL(@"低"), @"confidence": HUDL(@"中等置信度 · 其他程序")};
    if ([code isEqualToString:@"cpu_high"]) return @{@"bottleneck": HUDL(@"CPU偏高"), @"impact": HUDL(@"待判断"), @"confidence": HUDL(@"低置信度")};
    if ([code isEqualToString:@"thermal"]) return @{@"bottleneck": HUDL(@"热压力"), @"impact": HUDL(@"待判断"), @"confidence": HUDL(@"低置信度")};
    return @{@"bottleneck": HUDL(@"未发现"), @"impact": HUDL(@"较低"), @"confidence": HUDL(@"仅依据已监测指标")};
}
...
- (void)updateDisplay {
    ...
    self.hudView.bottleneckCard.subtitleLabel.stringValue = HUDL(@"当前判断");
    ...
    self.hudView.homeBottleneckCard.subtitleLabel.stringValue = [NSString stringWithFormat:HUDL(@"Codex影响 %@ · %@"), diagnosis[@"impact"], diagnosis[@"confidence"]];
    ...
    self.hudView.cpuCard.subtitleLabel.stringValue = HUDL(@"整机总算力占用");
    ...
    self.hudView.memoryPressureCard.subtitleLabel.stringValue = [NSString stringWithFormat:HUDL(@"已用 %.0f%% · 压力 %@"), snapshot.systemMemoryUsedPercent, snapshot.memoryPressureText ?: HUDL(@"未知")];
    ...
    self.hudView.homeSystemCard.valueLabel.stringValue = [NSString stringWithFormat:HUDL(@"CPU %.0f%% · 内存 %.0f%%"), snapshot.systemCPUPercent, snapshot.systemMemoryUsedPercent];
    self.hudView.homeSystemCard.subtitleLabel.stringValue = [NSString stringWithFormat:HUDL(@"已用 %.1f / %.0fG · 压力 %@"), snapshot.systemMemoryUsedGiB, snapshot.totalMemoryGiB, snapshot.memoryPressureText ?: HUDL(@"未知")];
    self.hudView.homeSystemCard.valueLabel.textColor = snapshot.systemCPUPercent >= 75 ? NSColor.systemOrangeColor : [self accentColor];
    self.hudView.attributionLabel.stringValue = [NSString stringWithFormat:HUDL(@"Codex：CPU %.1f%%（整机） · 内存 %.1fG（占总内存 %.0f%%）"), snapshot.codexCPUPercent, snapshot.codexMemoryGiB, snapshot.codexMemoryPercent];
    ...
    self.hudView.healthLabel.stringValue = [NSString stringWithFormat:HUDL(@"系统  Swap %.1fG (%+.0fM/10分)  ·  热状态 %@"), snapshot.swapUsedGiB, snapshot.swapDelta10MinMiB, snapshot.thermalText];
    double sum = 0, peak = 0;
    ...
    if (sampled >= 570) self.hudView.trendTitleLabel.stringValue = HUDL(@"CPU近10分钟");
    else if (sampled < 60) self.hudView.trendTitleLabel.stringValue = [NSString stringWithFormat:HUDL(@"CPU已采样%.0f秒"), MAX(0, sampled)];
    else self.hudView.trendTitleLabel.stringValue = [NSString stringWithFormat:HUDL(@"CPU已采样%ld分"), (long)MAX(1, floor(sampled / 60.0))];
    self.hudView.homeTrendTitleLabel.stringValue = self.hudView.trendTitleLabel.stringValue;
    self.hudView.trendLabel.stringValue = [NSString stringWithFormat:HUDL(@"平均%.0f%% · 峰值%.0f%%"), average, peak];
    self.hudView.homeTrendLabel.stringValue = self.hudView.trendLabel.stringValue;
    self.hudView.computerFreshnessLabel.stringValue = [NSString stringWithFormat:HUDL(@"来源  macOS系统接口 · 刚刚更新 · 每%.0f秒"), self.refreshInterval];
...
- (NSString *)visibleCodexStatus:(CodexStatusSnapshot *)s fiveHour:(BOOL)fiveHour weekly:(BOOL)weekly plan:(BOOL)plan usage:(BOOL)usage model:(BOOL)model activity:(BOOL)activity recent:(BOOL)recent {
    ...
    if (activity && s.activityErrorText.length > 0) return HUDL(@"任务活动暂时无法可靠判断");
    if (recent && s.recentTasksErrorText.length > 0) return s.recentTasksErrorText;
    if (quotaVisible && s.ordinaryUsageAllowed && !s.ordinaryUsageAllowed.boolValue && !HUDTimestampIsStale(s.ordinaryUsageUpdatedAt, 900)) return HUDL(@"官方：普通包含用量暂不可用");
    if (!s.quotaAvailable && !s.accountAvailable && !s.usageAvailable && !(activity && s.activityAvailable) && !(recent && s.recentTasksAvailable)) return s.statusText ?: HUDL(@"正在连接本机Codex");
    if (fiveHour && !s.fiveHourAvailable) return HUDL(@"5小时额度暂未返回");
    if (weekly && !s.weeklyAvailable) return HUDL(@"每周额度暂未返回");
    if (plan && !s.accountAvailable) return HUDL(@"订阅信息暂未返回");
    if (usage && !s.usageAvailable) return HUDL(@"Token用量暂未返回");
    if (model && !s.modelQuotaAvailable) return HUDL(@"模型专属额度暂未返回");
    if (activity && !s.activityAvailable) return HUDL(@"任务活动暂未返回");
    if (recent && !s.recentTasksAvailable) return HUDL(@"最近任务暂未返回");
    ...
    if (accountStale || activityStale || recentStale) return HUDL(@"部分数据已过期，显示上次数据");
    return HUDL(@"Codex数据正常");
}
...
- (NSString *)freshnessText:(CodexStatusSnapshot *)s fiveHour:(BOOL)fiveHour weekly:(BOOL)weekly plan:(BOOL)plan usage:(BOOL)usage model:(BOOL)model activity:(BOOL)activity recent:(BOOL)recent {
    ...
    if (fiveHour || weekly || model) {
        NSString *name = fiveHour && weekly ? HUDL(@"额度") : (fiveHour ? HUDL(@"5小时") : (weekly ? HUDL(@"每周") : HUDL(@"模型额度")));
        [parts addObject:[NSString stringWithFormat:@"%@ %@", name, FormatModuleState(s.quotaUpdatedAt, s.quotaErrorText, 900)]];
    }
    if (plan) [parts addObject:[NSString stringWithFormat:HUDL(@"订阅 %@"), FormatModuleState(s.accountUpdatedAt, s.accountErrorText, 900)]];
    if (usage) [parts addObject:[NSString stringWithFormat:HUDL(@"用量 %@"), FormatModuleState(s.usageUpdatedAt, s.usageErrorText, 900)]];
    if (activity) [parts addObject:[NSString stringWithFormat:HUDL(@"任务活动 %@"), FormatModuleState(s.activityUpdatedAt, s.activityErrorText, 60)]];
    if (recent) [parts addObject:[NSString stringWithFormat:HUDL(@"最近任务 %@"), FormatModuleState(s.recentTasksUpdatedAt, s.recentTasksErrorText, 900)]];
    return parts.count ? [parts componentsJoinedByString:@" · "] : HUDL(@"未启用Codex显示模块");
}
...
- (void)updateCodexDisplay {
    ...
    if (self.showLocalCost && !pageHasOfficialData) statusText = s.localCostAvailable ? HUDL(@"本机Token历史正常") : (s.localCostErrorText ?: HUDL(@"正在读取本机Token历史"));
    else if (self.showLocalCost && s.localCostErrorText.length > 0) statusText = HUDL(@"本机Token历史读取失败");
    ...
    if (self.homeShowLocalCost && !homeHasOfficialData) homeStatusText = s.localCostAvailable ? HUDL(@"本机Token历史正常") : (s.localCostErrorText ?: HUDL(@"正在读取本机Token历史"));
    else if (self.homeShowLocalCost && s.localCostErrorText.length > 0) homeStatusText = HUDL(@"本机Token历史读取失败");
    ...
    self.hudView.homePlanCard.valueLabel.textColor = self.hudView.planCard.valueLabel.textColor;
    self.hudView.homePlanCard.subtitleLabel.stringValue = self.hudView.planCard.subtitleLabel.stringValue;
    NSString *activityValue = s.activityErrorText.length > 0 ? HUDL(@"当前无法可靠判断") : HUDL(@"当前未返回");
    NSString *activitySubtitle = s.activityErrorText ?: HUDL(@"本机会话记录 · 活跃5秒 · 空闲20秒");
    if (s.activityAvailable) {
        if (s.activeTaskCount > 0) activityValue = [NSString stringWithFormat:s.activityPartial ? HUDL(@"%ld个可确认活跃 · 最长%@") : HUDL(@"%ld个活跃 · 最长%@"), (long)s.activeTaskCount, FormatDuration(s.longestActiveTaskSec)];
        else activityValue = s.activityPartial ? HUDL(@"暂无可确认的活跃任务") : HUDL(@"当前没有活跃任务");
        ...
        else activitySubtitle = HUDL(@"本机活动推测 · 活跃5秒 · 空闲20秒");
    }
    ...
    for (NSDictionary<NSString *, id> *task in s.recentTasks ?: @[]) {
        NSString *name = [task[@"name"] isKindOfClass:NSString.class] ? task[@"name"] : HUDL(@"未命名任务");
    ...
    NSString *recentFooter = s.recentTasksErrorText.length > 0 ? HUDL(@"更新失败 · 显示上次列表") : HUDL(@"官方任务历史 · 不代表正在运行");
    ...
    if (s.usageAvailable) {
        self.hudView.usageCard.valueLabel.stringValue = s.todayUsageAvailable ? [NSString stringWithFormat:HUDL(@"今日 %@"), FormatTokens(s.todayTokens)] : (s.latestUsageDate.length > 0 ? [NSString stringWithFormat:@"%@ %@", FormatUsageDate(s.latestUsageDate), FormatTokens(s.latestUsageTokens)] : HUDL(@"今日数据未返回"));
        NSString *trend = @"";
        ...
        NSString *through = s.todayUsageAvailable ? @"" : (s.latestUsageDate.length > 0 ? HUDL(@"今日数据未返回 · ") : @"");
        self.hudView.usageCard.subtitleLabel.stringValue = s.usageErrorText.length > 0 ? [NSString stringWithFormat:HUDL(@"%@7天 %@%@ · 保留上次数据"), through, FormatTokens(s.sevenDayTokens), trend] : [NSString stringWithFormat:HUDL(@"%@7天 %@%@ · 不等于额度"), through, FormatTokens(s.sevenDayTokens), trend];
        ...
        self.hudView.usageCard.valueLabel.stringValue = HUDL(@"当前未返回");
        self.hudView.usageCard.subtitleLabel.stringValue = s.usageErrorText ?: HUDL(@"不会换算成额度百分比");
        self.hudView.homeUsageCard.valueLabel.stringValue = self.hudView.usageCard.valueLabel.stringValue;
        self.hudView.homeUsageCard.subtitleLabel.stringValue = self.hudView.usageCard.subtitleLabel.stringValue;
    }
    if (s.localCostAvailable) {
        if (self.showLocalCost || self.homeShowLocalCost) HUDRefreshExchangeRates();
        self.hudView.localCostCard.titleLabel.stringValue = HUDL(@"安装后近30天 · Token与API估算");
        self.hudView.homeLocalCostCard.titleLabel.stringValue = self.hudView.localCostCard.titleLabel.stringValue;
        NSString *costValue = [NSString stringWithFormat:@"%@ · %@", FormatTokens(s.localThirtyDayTokens), FormatUSD(s.localThirtyDayCostUSD)];
        NSMutableString *costSubtitle = [[NSString stringWithFormat:HUDL(@"今日 %@ · 近7天 %@ · 本月趋势 %@（估算）"), FormatTokens(s.localTodayTokens), FormatTokens(s.localSevenDayTokens), FormatUSD(s.localMonthForecastCostUSD)] mutableCopy];
        if (s.localPricedTokenPercent < 99.5) [costSubtitle appendFormat:HUDL(@" · 计价覆盖%.0f%%"), s.localPricedTokenPercent];
        if (s.localCostScanIncomplete) [costSubtitle appendString:HUDL(@" · 模型样本更新中")];
        ...
        NSString *costHelp = [NSString stringWithFormat:@"%@\n%@\n%@", costValue, costSubtitle,
            HUDL(@"金额是按参考API标准价格折算的估值，不是订阅实际账单；未包含Fast等加价。子任务继承量不重复计入，无法可靠计价的记录不猜价格。")];
        self.hudView.localCostCard.toolTip = costHelp;
        self.hudView.homeLocalCostCard.toolTip = costHelp;
    } else {
        NSString *costState = s.localCostErrorText.length > 0 ? HUDL(@"当前不可用") : HUDL(@"正在读取");
        NSString *costDetail = s.localCostErrorText ?: HUDL(@"只统计安装软件后的新增记录");
    ...
    if (s.fiveHourForecastAvailable) {
        [forecastHeads addObject:[NSString stringWithFormat:HUDL(@"5小时 %@"), FormatForecastHeadline(s.fiveHourForecastHeadline)]];
        if (s.fiveHourForecastDetail.length > 0) [forecastDetails addObject:[NSString stringWithFormat:HUDL(@"5小时 %@"), s.fiveHourForecastDetail]];
    }
    if (s.weeklyForecastAvailable) {
        [forecastHeads addObject:[NSString stringWithFormat:HUDL(@"每周 %@"), FormatForecastHeadline(s.weeklyForecastHeadline)]];
        if (s.weeklyForecastDetail.length > 0) [forecastDetails addObject:[NSString stringWithFormat:HUDL(@"每周 %@"), s.weeklyForecastDetail]];
    }
    NSString *forecastValue = forecastHeads.count > 0 ? [forecastHeads componentsJoinedByString:@" · "] : HUDL(@"正在积累历史");
    NSString *forecastSubtitle = forecastDetails.count > 0 ? [forecastDetails componentsJoinedByString:@" · "] : (s.weeklyForecastDetail.length > 0 ? s.weeklyForecastDetail : (s.fiveHourForecastDetail.length > 0 ? s.fiveHourForecastDetail : HUDL(@"至少需要15分钟数据")));
    BOOL forecastRisk = [s.fiveHourForecastHeadline isEqualToString:HUDL(@"可能提前用完")] ||
                        [s.weeklyForecastHeadline isEqualToString:HUDL(@"可能提前用完")];
    NSColor *forecastColor = forecastRisk ? NSColor.systemOrangeColor : [self accentColor];
    ...
    self.hudView.longestTurnCard.valueLabel.stringValue = s.longestRunningTurnAvailable ? FormatDuration(s.longestRunningTurnSec) : HUDL(@"当前未返回");
    self.hudView.longestTurnCard.subtitleLabel.stringValue = s.usageErrorText.length > 0 ? HUDL(@"部分未更新 · 显示上次数据") : HUDL(@"账户历史最长");
    ...
    self.hudView.longestStreakCard.valueLabel.stringValue = s.longestStreakAvailable ? [NSString stringWithFormat:HUDL(@"%ld天"), (long)s.longestStreakDays] : HUDL(@"当前未返回");
    self.hudView.longestStreakCard.subtitleLabel.stringValue = s.usageErrorText.length > 0 ? HUDL(@"部分未更新 · 显示上次数据") : HUDL(@"账户历史最长");
    ...
    self.hudView.peakDailyTokensCard.valueLabel.stringValue = s.peakDailyTokensAvailable ? FormatTokens(s.peakDailyTokens) : HUDL(@"当前未返回");
    self.hudView.peakDailyTokensCard.subtitleLabel.stringValue = s.usageErrorText.length > 0 ? HUDL(@"部分未更新 · 显示上次数据") : HUDL(@"官方账户历史峰值");
    ...
    if (s.modelQuotaAvailable) {
        ...
        self.hudView.modelQuotaCard.subtitleLabel.stringValue = [NSString stringWithFormat:@"%@ · %@", s.modelQuotaWindowLabel ?: HUDL(@"独立额度"), FormatReset(s.modelQuotaResetAt)];
    ...
    if (self.showLocalCost || self.showTokenWindows) {
        NSString *costFreshness = [NSString stringWithFormat:HUDL(@"本机Token %@"), FormatModuleState(s.localCostUpdatedAt, s.localCostErrorText, 900)];
        pageFreshness = [pageFreshness isEqualToString:HUDL(@"未启用Codex显示模块")] ? costFreshness : [NSString stringWithFormat:@"%@ · %@", pageFreshness, costFreshness];
    }
    ...
    if (self.homeShowLocalCost || self.homeShowTokenWindows) {
        NSString *costFreshness = [NSString stringWithFormat:HUDL(@"本机Token %@"), FormatModuleState(s.localCostUpdatedAt, s.localCostErrorText, 900)];
        homeCodexFreshness = [homeCodexFreshness isEqualToString:HUDL(@"未启用Codex显示模块")] ? costFreshness : [NSString stringWithFormat:@"%@ · %@", homeCodexFreshness, costFreshness];
    }
...
- (void)updateTokenWindowDisplay:(CodexStatusSnapshot *)s {
    ...
    void (^fill)(HUDMetricCard *, NSDictionary *, NSString *, double, BOOL) = ^(HUDMetricCard *card, NSDictionary *summary, NSString *extra, double officialUsed, BOOL baseline) {
        if (![summary[@"available"] boolValue]) {
            card.valueLabel.stringValue = HUDL(@"正在积累");
            card.subtitleLabel.stringValue = HUDL(@"本地统计 · 暂无完整时间点");
        ...
        NSMutableArray<NSString *> *parts = [NSMutableArray array];
        if (baseline) [parts addObject:weeklyComparisonAvailable ? HUDL(@"周基准100%") : HUDL(@"周基准待完整")];
        else if (weeklyComparisonAvailable && weeklyTokens > 0) [parts addObject:[NSString stringWithFormat:HUDL(@"周占%.1f%%"), (double)tokens / (double)weeklyTokens * 100.0]];
        else [parts addObject:HUDL(@"周占待完整")];
        if (officialUsed >= 0) [parts addObject:[NSString stringWithFormat:HUDL(@"额度%.0f%%"), officialUsed]];
        [parts addObject:complete ? HUDL(@"本地完整") : HUDL(@"本地局部")];
    ...
    fill(self.hudView.fiveHourTokensCard, five ?: @{}, HUDL(@"约5分钟精度"), fiveOfficialUsed, NO);
    fill(self.hudView.rollingDayTokensCard, day, HUDL(@"滚动24h"), -1, NO);
    fill(self.hudView.weeklyTokensCard, week ?: @{}, HUDL(@"官方周期"), weeklyOfficialUsed, YES);
    if (!five) self.hudView.fiveHourTokensCard.subtitleLabel.stringValue = HUDL(@"等待官方短周期 · 本地统计");
    if (!week) self.hudView.weeklyTokensCard.subtitleLabel.stringValue = HUDL(@"等待官方每周周期 · 本地统计");
    for (NSArray *pair in @[
...
- (void)updateQuotaDetailsDisplay:(CodexStatusSnapshot *)s {
    ...
    for (NSDictionary<NSString *, id> *bucket in s.rateLimitBuckets ?: @[]) {
        NSString *name = [bucket[@"name"] isKindOfClass:NSString.class] ? bucket[@"name"] : HUDL(@"额度");
        ...
        [rows addObject:[NSString stringWithFormat:HUDL(@"%@ · %@ · 剩余%.0f%% · %@"), name, FormatQuotaWindow(duration), remaining, FormatReset(reset)]];
    }
    NSMutableArray<NSString *> *footer = [NSMutableArray arrayWithObject:HUDL(@"官方完整额度列表")];
    if (s.rateLimitResetCreditsAvailable) [footer addObject:[NSString stringWithFormat:HUDL(@"可用恢复次数%ld"), (long)s.rateLimitResetCreditsCount]];
    else [footer addObject:HUDL(@"恢复次数未返回")];
    if (s.rateLimitReachedType.length > 0) [footer addObject:[NSString stringWithFormat:HUDL(@"官方触顶：%@"), s.rateLimitReachedType]];
    if (s.ordinaryUsageAllowed && !HUDTimestampIsStale(s.ordinaryUsageUpdatedAt, 900)) [footer addObject:s.ordinaryUsageAllowed.boolValue ? HUDL(@"普通包含用量允许") : HUDL(@"普通包含用量暂不可用")];
...
- (void)evaluateWeeklyConsumptionAlert:(CodexStatusSnapshot *)s {
    ...
    if (!available) return;
    NSString *period = naturalDay ? HUDL(@"今日") : HUDL(@"最近24小时");
    NSString *alertText = [NSString stringWithFormat:HUDL(@"%@已用周额度%.1f%% · 阈值%.0f%%"), period, consumed, self.weeklyConsumptionAlertThreshold];
...
- (void)updateDetailLabels {
    ...
    if (self.currentPage == 0) {
        CodexStatusSnapshot *s = self.codexProvider.snapshot;
        if (self.homeShowTaskActivity && row < 5) { self.hudView.detailLabels[row].stringValue = s.activityAvailable ? [NSString stringWithFormat:HUDL(@"任务活动   %ld个%@ · 最长%@ · 本机推测"), (long)s.activeTaskCount, s.activityPartial ? HUDL(@"可确认活跃") : HUDL(@"活跃"), FormatDuration(s.longestActiveTaskSec)] : HUDL(@"任务活动   当前无法可靠判断"); self.hudView.detailLabels[row++].hidden = NO; }
        if (self.homeShowRecentTasks && row < 5) { self.hudView.detailLabels[row].stringValue = s.recentTasksAvailable ? [NSString stringWithFormat:HUDL(@"最近任务   已读取%ld项 · 历史列表"), (long)s.recentTaskCount] : HUDL(@"最近任务   当前未返回"); self.hudView.detailLabels[row++].hidden = NO; }
        if (self.homeShowServiceStatus && row < 5) { HUDOpenAIServiceStatusSnapshot *service = self.serviceStatusProvider.snapshot; self.hudView.detailLabels[row].stringValue = service.available ? [NSString stringWithFormat:HUDL(@"官方状态   %@ · %@"), service.headline, FormatAge(service.updatedAt)] : HUDL(@"官方状态   正在读取"); self.hudView.detailLabels[row++].hidden = NO; }
        ...
        if (self.homeShowLongestTurn && row < 5) { self.hudView.detailLabels[row].stringValue = s.longestRunningTurnAvailable ? [NSString stringWithFormat:HUDL(@"最长单次   %@"), FormatDuration(s.longestRunningTurnSec)] : HUDL(@"最长单次   当前接口未返回"); self.hudView.detailLabels[row++].hidden = NO; }
        if (self.homeShowLongestStreak && row < 5) { self.hudView.detailLabels[row].stringValue = s.longestStreakAvailable ? [NSString stringWithFormat:HUDL(@"最长连续   %ld天"), (long)s.longestStreakDays] : HUDL(@"最长连续   当前接口未返回"); self.hudView.detailLabels[row++].hidden = NO; }
        NativeSnapshot *n = self.lastSnapshot;
        if ((self.homeShowDiagnosis || self.homeShowSystem) && row < 5) { self.hudView.detailLabels[row].stringValue = n ? [NSString stringWithFormat:HUDL(@"电脑摘要   CPU %.0f%% · 内存 %.1f/%.0fG (%.0f%%) · 压力 %@"), n.systemCPUPercent, n.systemMemoryUsedGiB, n.totalMemoryGiB, n.systemMemoryUsedPercent, n.memoryPressureText] : HUDL(@"电脑摘要   正在采样"); self.hudView.detailLabels[row++].hidden = NO; }
        if (self.homeShowAttribution && row < 5) { self.hudView.detailLabels[row].stringValue = n ? [NSString stringWithFormat:HUDL(@"Codex摘要  CPU %.1f%%（整机） · 内存 %.1fG（总量%.0f%%）"), n.codexCPUPercent, n.codexMemoryGiB, n.codexMemoryPercent] : HUDL(@"Codex摘要  正在采样"); self.hudView.detailLabels[row++].hidden = NO; }
        if (row < 5) { self.hudView.detailLabels[row].stringValue = HUDL(@"齿轮可一次查看并修改全部显示选项"); self.hudView.detailLabels[row++].hidden = NO; }
    } else if (self.currentPage == 1) {
        CodexStatusSnapshot *s = self.codexProvider.snapshot;
        if (self.showTaskActivity && row < 5) { self.hudView.detailLabels[row].stringValue = s.activityAvailable ? [NSString stringWithFormat:HUDL(@"任务活动   %ld个%@ · 最长%@ · 本机推测"), (long)s.activeTaskCount, s.activityPartial ? HUDL(@"可确认活跃") : HUDL(@"活跃"), FormatDuration(s.longestActiveTaskSec)] : HUDL(@"任务活动   当前无法可靠判断"); self.hudView.detailLabels[row++].hidden = NO; }
        if (self.showRecentTasks && row < 5) { self.hudView.detailLabels[row].stringValue = s.recentTasksAvailable ? [NSString stringWithFormat:HUDL(@"最近任务   已读取%ld项 · 历史列表"), (long)s.recentTaskCount] : HUDL(@"最近任务   当前未返回"); self.hudView.detailLabels[row++].hidden = NO; }
        ...
        if (self.showLongestTurn && row < 5) { self.hudView.detailLabels[row].stringValue = s.longestRunningTurnAvailable ? [NSString stringWithFormat:HUDL(@"最长单次   %@"), FormatDuration(s.longestRunningTurnSec)] : HUDL(@"最长单次   当前接口未返回"); self.hudView.detailLabels[row++].hidden = NO; }
        if (self.showLongestStreak && row < 5) { self.hudView.detailLabels[row].stringValue = s.longestStreakAvailable ? [NSString stringWithFormat:HUDL(@"最长连续   %ld天"), (long)s.longestStreakDays] : HUDL(@"最长连续   当前接口未返回"); self.hudView.detailLabels[row++].hidden = NO; }
        if (self.showModelQuota && row < 5) { self.hudView.detailLabels[row].stringValue = s.modelQuotaAvailable ? [NSString stringWithFormat:HUDL(@"模型额度   %@ %.0f%%"), s.modelQuotaName, s.modelQuotaRemainingPercent] : HUDL(@"模型额度   当前未返回"); self.hudView.detailLabels[row++].hidden = NO; }
        if (row < 5) { self.hudView.detailLabels[row].stringValue = HUDL(@"数据来源   Codex本机接口 · 活跃5秒/空闲20秒 · 账户1–5分钟"); self.hudView.detailLabels[row++].hidden = NO; }
    } else if (self.lastSnapshot) {
        NativeSnapshot *s = self.lastSnapshot;
        if (self.showAttribution && row < 5) { self.hudView.detailLabels[row].stringValue = [NSString stringWithFormat:HUDL(@"Codex %ld进程 · 渲染%ld · 工具%ld · 最大 %.1fG"), (long)s.codexProcessCount, (long)s.codexRendererCount, (long)s.codexHelperCount, s.codexLargestGiB]; self.hudView.detailLabels[row++].hidden = NO; }
        if (self.showAttribution && row < 5) { self.hudView.detailLabels[row].stringValue = [NSString stringWithFormat:HUDL(@"Codex磁盘  读 %@ · 写 %@"), FormatRate(s.codexDiskReadMBps), FormatRate(s.codexDiskWriteMBps)]; self.hudView.detailLabels[row++].hidden = NO; }
        if (self.showSystem && row < 5) { self.hudView.detailLabels[row].stringValue = [NSString stringWithFormat:HUDL(@"整机内存  %.1f/%.0fG (%.0f%%) · 压缩 %.1fG"), s.systemMemoryUsedGiB, s.totalMemoryGiB, s.systemMemoryUsedPercent, s.compressedGiB]; self.hudView.detailLabels[row++].hidden = NO; }
        if (self.showSystem && row < 5) { self.hudView.detailLabels[row].stringValue = [NSString stringWithFormat:HUDL(@"CPU前列  %@"), [self topAppsText:s.topCPUApps cpu:YES]]; self.hudView.detailLabels[row++].hidden = NO; }
        if (self.showMemoryApps && row < 5) { self.hudView.detailLabels[row].stringValue = [NSString stringWithFormat:HUDL(@"内存前列 %@"), [self topAppsText:s.topMemoryApps cpu:NO]]; self.hudView.detailLabels[row++].hidden = NO; }
    }
}
...
- (void)chooseSubscriptionDate:(id)sender {
    ...
    picker.dateValue = HUDSubscriptionDateFromString(self.subscriptionDateString) ?: NSDate.date;
    NSAlert *alert = [NSAlert new];
    alert.messageText = HUDL(@"填写本期订阅截止/续费日");
    alert.informativeText = HUDL(@"请在官方账单页核对后填写。日期只保存在这台电脑。");
    alert.accessoryView = picker;
    [alert addButtonWithTitle:HUDL(@"保存")];
    [alert addButtonWithTitle:HUDL(@"取消")];
    __weak typeof(self) weakSelf = self;
    ...
    if (self.settingsWindow) [alert beginSheetModalForWindow:self.settingsWindow completionHandler:completion];
...
- (void)openOfficialBillingPage:(id)sender {
    NSURL *url = HUDOfficialBillingURL();
    if (!url || ![NSWorkspace.sharedWorkspace openURL:url]) NSBeep();
}
...
- (void)performUpdateCheckManual:(BOOL)manual {
    ...
    self.updateStatusLabel.stringValue = HUDL(@"正在检查 GitHub 新版本…");
    __weak typeof(self) weakSelf = self;
    [self.updateManager checkForUpdates:^(HUDUpdateCheckResult result, HUDReleaseInfo *release, NSString *message) {
        ...
        if (result == HUDUpdateCheckResultAvailable) {
            ...
            NSAlert *alert = [NSAlert new];
            alert.messageText = result == HUDUpdateCheckResultUpToDate ? HUDL(@"已经是最新版") : HUDL(@"暂时无法检查更新");
            alert.informativeText = message;
            [alert addButtonWithTitle:HUDL(@"好")];
            [alert runModal];
        }
    }];
}
...
- (void)presentUpdatePrompt:(HUDReleaseInfo *)release {
    NSAlert *alert = [NSAlert new];
    alert.messageText = [NSString stringWithFormat:HUDL(@"发现 Codex Monitor HUD %@"), release.version];
    NSString *notes = release.releaseNotes.length ? release.releaseNotes : HUDL(@"新版本已经可以下载。");
    alert.informativeText = notes.length > 700 ? [[notes substringToIndex:700] stringByAppendingString:@"…"] : notes;
    [alert addButtonWithTitle:HUDL(@"一键更新")];
    [alert addButtonWithTitle:HUDL(@"稍后")];
    [alert addButtonWithTitle:HUDL(@"查看发布页面")];
...
- (void)installRelease:(HUDReleaseInfo *)release {
    ...
    self.updateButton.enabled = NO;
    self.updateStatusLabel.stringValue = HUDL(@"正在下载并验证更新…");
    __weak typeof(self) weakSelf = self;
    [self.updateManager installRelease:release completion:^(BOOL prepared, NSString *message) {
        ...
        weakSelf.updateButton.enabled = YES;
        NSAlert *alert = [NSAlert new];
        alert.messageText = HUDL(@"一键更新未完成");
        alert.informativeText = message;
        [alert addButtonWithTitle:HUDL(@"打开发布页面")];
        [alert addButtonWithTitle:HUDL(@"取消")];
...
- (NSButton *)settingsCheckbox:(NSString *)title action:(SEL)action state:(BOOL)state {
    ...
    button.toolTip = state ? HUDL(@"当前显示，点击后隐藏") : HUDL(@"当前隐藏，点击后显示");
    return button;
}
...
- (NSView *)settingsContentView {
    ...
    NSArray<NSView *> *homeControls = @[
        [self settingsCheckbox:HUDL(@"当前任务活动（本机推测）") action:@selector(toggleHomeTaskActivity:) state:self.homeShowTaskActivity],
        [self settingsCheckbox:HUDL(@"最近任务（历史）") action:@selector(toggleHomeRecentTasks:) state:self.homeShowRecentTasks],
        [self settingsCheckbox:HUDL(@"5小时额度") action:@selector(toggleHomeFiveHour:) state:self.homeShowFiveHour],
        [self settingsCheckbox:HUDL(@"每周额度") action:@selector(toggleHomeWeekly:) state:self.homeShowWeekly],
        [self settingsCheckbox:HUDL(@"订阅类型") action:@selector(toggleHomePlan:) state:self.homeShowPlan],
        [self settingsCheckbox:HUDL(@"账户Token统计") action:@selector(toggleHomeUsage:) state:self.homeShowUsage],
        [self settingsCheckbox:HUDL(@"5小时/24小时/每周Token对比") action:@selector(toggleHomeTokenWindows:) state:self.homeShowTokenWindows],
        [self settingsCheckbox:HUDL(@"Token用量与费用") action:@selector(toggleHomeLocalCost:) state:self.homeShowLocalCost],
        [self settingsCheckbox:HUDL(@"额度趋势预测") action:@selector(toggleHomeQuotaForecast:) state:self.homeShowQuotaForecast],
        [self settingsCheckbox:HUDL(@"OpenAI服务状态") action:@selector(toggleHomeServiceStatus:) state:self.homeShowServiceStatus],
        [self settingsCheckbox:HUDL(@"完整额度列表与恢复次数") action:@selector(toggleHomeQuotaDetails:) state:self.homeShowQuotaDetails],
        [self settingsCheckbox:HUDL(@"模型专属额度") action:@selector(toggleHomeModelQuota:) state:self.homeShowModelQuota],
        [self settingsCheckbox:HUDL(@"最长单次任务时长") action:@selector(toggleHomeLongestTurn:) state:self.homeShowLongestTurn],
        [self settingsCheckbox:HUDL(@"历史最长连续天数") action:@selector(toggleHomeLongestStreak:) state:self.homeShowLongestStreak],
        [self settingsCheckbox:HUDL(@"历史单日峰值Token") action:@selector(toggleHomePeakDailyTokens:) state:self.homeShowPeakDailyTokens],
        [self settingsCheckbox:HUDL(@"瓶颈判断") action:@selector(toggleHomeDiagnosis:) state:self.homeShowDiagnosis],
        [self settingsCheckbox:HUDL(@"电脑核心状态") action:@selector(toggleHomeSystem:) state:self.homeShowSystem],
        [self settingsCheckbox:HUDL(@"Codex性能占用") action:@selector(toggleHomeAttribution:) state:self.homeShowAttribution],
        [self settingsCheckbox:HUDL(@"内存占用排行") action:@selector(toggleHomeMemoryApps:) state:self.homeShowMemoryApps],
        [self settingsCheckbox:HUDL(@"CPU趋势") action:@selector(toggleHomeTrend:) state:self.homeShowTrend]
    ];
    NSArray<NSView *> *codexControls = @[
        [self settingsCheckbox:HUDL(@"当前任务活动（本机推测）") action:@selector(toggleTaskActivity:) state:self.showTaskActivity],
        [self settingsCheckbox:HUDL(@"最近任务（历史）") action:@selector(toggleRecentTasks:) state:self.showRecentTasks],
        [self settingsCheckbox:HUDL(@"5小时额度") action:@selector(toggleFiveHourQuota:) state:self.showFiveHourQuota],
        [self settingsCheckbox:HUDL(@"每周额度") action:@selector(toggleWeeklyQuota:) state:self.showWeeklyQuota],
        [self settingsCheckbox:HUDL(@"订阅类型") action:@selector(togglePlan:) state:self.showPlan],
        [self settingsCheckbox:HUDL(@"账户Token统计") action:@selector(toggleUsage:) state:self.showUsage],
        [self settingsCheckbox:HUDL(@"5小时/24小时/每周Token对比") action:@selector(toggleTokenWindows:) state:self.showTokenWindows],
        [self settingsCheckbox:HUDL(@"Token用量与费用") action:@selector(toggleLocalCost:) state:self.showLocalCost],
        [self settingsCheckbox:HUDL(@"额度趋势预测") action:@selector(toggleQuotaForecast:) state:self.showQuotaForecast],
        [self settingsCheckbox:HUDL(@"OpenAI服务状态") action:@selector(toggleServiceStatus:) state:self.showServiceStatus],
        [self settingsCheckbox:HUDL(@"完整额度列表与恢复次数") action:@selector(toggleQuotaDetails:) state:self.showQuotaDetails],
        [self settingsCheckbox:HUDL(@"模型专属额度") action:@selector(toggleModelQuota:) state:self.showModelQuota],
        [self settingsCheckbox:HUDL(@"最长单次任务时长") action:@selector(toggleLongestTurn:) state:self.showLongestTurn],
        [self settingsCheckbox:HUDL(@"历史最长连续天数") action:@selector(toggleLongestStreak:) state:self.showLongestStreak],
        [self settingsCheckbox:HUDL(@"历史单日峰值Token") action:@selector(togglePeakDailyTokens:) state:self.showPeakDailyTokens]
    ];
    NSArray<NSView *> *computerControls = @[
        [self settingsCheckbox:HUDL(@"电脑核心状态") action:@selector(toggleSystem:) state:self.showSystem],
        [self settingsCheckbox:HUDL(@"Codex性能占用") action:@selector(toggleAttribution:) state:self.showAttribution],
        [self settingsCheckbox:HUDL(@"内存占用排行") action:@selector(toggleMemoryApps:) state:self.showMemoryApps],
        [self settingsCheckbox:HUDL(@"10分钟CPU趋势") action:@selector(toggleTrend:) state:self.showTrend]
    ];
    NSBox *homeBox = [self settingsGroup:HUDL(@"主页显示（勾选 = 显示）") controls:homeControls];
    NSBox *codexBox = [self settingsGroup:HUDL(@"Codex页面") controls:codexControls];
    NSBox *computerBox = [self settingsGroup:HUDL(@"电脑性能页面") controls:computerControls];
    ...
    HUDModuleOrderController *codexOrderController = [[HUDModuleOrderController alloc] initWithOrderKey:@"codex" items:HUDOrderItems(self.homeCodexModuleOrder, @{
        @"activity": HUDL(@"任务活动"), @"recent": HUDL(@"最近任务"), @"quota": HUDL(@"5小时与每周额度"),
        @"insights": HUDL(@"订阅与账户Token统计"), @"tokenWindows": HUDL(@"5小时/24小时/每周Token对比"), @"cost": HUDL(@"Token用量与费用"),
        @"forecast": HUDL(@"额度趋势预测"), @"service": HUDL(@"OpenAI服务状态"), @"quotaDetails": HUDL(@"完整额度列表"), @"history": HUDL(@"历史峰值与连续使用")
    ...
    HUDModuleOrderController *computerOrderController = [[HUDModuleOrderController alloc] initWithOrderKey:@"computer" items:HUDOrderItems(self.homeComputerModuleOrder, @{
        @"summary": HUDL(@"瓶颈与电脑状态"), @"attribution": HUDL(@"Codex性能占用"), @"memory": HUDL(@"内存占用排行"), @"trend": HUDL(@"CPU趋势")
    }) changed:^(NSArray<NSString *> *order) {
    ...
    NSTextField *codexOrderHint = [NSTextField labelWithString:HUDL(@"拖动 ≡ 调整主页中的上下顺序")];
    codexOrderHint.font = [NSFont systemFontOfSize:11]; codexOrderHint.textColor = NSColor.secondaryLabelColor;
    NSTextField *computerOrderHint = [NSTextField labelWithString:HUDL(@"拖动 ≡ 调整主页中的上下顺序")];
    computerOrderHint.font = [NSFont systemFontOfSize:11]; computerOrderHint.textColor = NSColor.secondaryLabelColor;
    NSBox *codexOrderBox = [self settingsGroup:HUDL(@"主页 · Codex模块排序") controls:@[codexOrderHint, codexOrderController.scrollView]];
    NSBox *computerOrderBox = [self settingsGroup:HUDL(@"主页 · 电脑模块排序") controls:@[computerOrderHint, computerOrderController.scrollView]];
    ...
    NSButton *weeklyAlertToggle = [self settingsCheckbox:HUDL(@"启用周额度消耗提醒（默认关闭）") action:@selector(toggleWeeklyConsumptionAlert:) state:self.weeklyConsumptionAlertEnabled];
    NSButton *weeklySystemNotificationToggle = [self settingsCheckbox:HUDL(@"同时发送静音系统通知（可选）") action:@selector(toggleWeeklyConsumptionSystemNotification:) state:self.weeklyConsumptionSystemNotificationEnabled];
    NSPopUpButton *weeklyAlertMode = [[NSPopUpButton alloc] initWithFrame:NSZeroRect pullsDown:NO];
    [weeklyAlertMode addItemWithTitle:HUDL(@"滚动24小时")];
    weeklyAlertMode.lastItem.representedObject = @"rolling24h";
    [weeklyAlertMode addItemWithTitle:HUDL(@"自然日（当天零点起）")];
    ...
    NSTextField *modeLabel = [NSTextField labelWithString:HUDL(@"统计方式")];
    ...
    self.weeklyConsumptionThresholdLabel = [NSTextField labelWithString:[NSString stringWithFormat:HUDL(@"提醒阈值：%.0f%%"), self.weeklyConsumptionAlertThreshold]];
    ...
    NSTextField *alertHint = [NSTextField wrappingLabelWithString:HUDL(@"阈值可选1%—100%；数据不足不提醒，同一周额度周期只提醒一次。系统通知复用现有额度刷新，不增加轮询。")];
    alertHint.font = [NSFont systemFontOfSize:11]; alertHint.textColor = NSColor.secondaryLabelColor;
    NSBox *weeklyAlertBox = [self settingsGroup:HUDL(@"周额度消耗提醒") controls:@[weeklyAlertToggle, weeklySystemNotificationToggle, modeRow, self.weeklyConsumptionThresholdLabel, weeklyAlertThreshold, alertHint]];

    self.subscriptionDateStatusLabel = [NSTextField labelWithString:@""];
    self.subscriptionDateStatusLabel.font = [NSFont systemFontOfSize:12.5 weight:NSFontWeightMedium];
    NSButton *setSubscriptionDate = [NSButton buttonWithTitle:HUDL(@"填写日期…") target:self action:@selector(chooseSubscriptionDate:)];
    setSubscriptionDate.bezelStyle = NSBezelStyleRounded;
    setSubscriptionDate.toolTip = HUDL(@"手动填写本期订阅截止或续费日期");
    NSButton *openBilling = [NSButton buttonWithTitle:HUDL(@"打开官方账单页") target:self action:@selector(openOfficialBillingPage:)];
    openBilling.bezelStyle = NSBezelStyleRounded;
    openBilling.toolTip = HUDL(@"在浏览器打开ChatGPT官方账单设置，不读取浏览器数据");
    self.subscriptionDateClearButton = [NSButton buttonWithTitle:HUDL(@"清除日期") target:self action:@selector(clearSubscriptionDate:)];
    self.subscriptionDateClearButton.bezelStyle = NSBezelStyleRounded;
    NSStackView *subscriptionDateButtons = [NSStackView stackViewWithViews:@[setSubscriptionDate, openBilling, self.subscriptionDateClearButton]];
    ...
    NSButton *resetSize = [NSButton buttonWithTitle:HUDL(@"恢复标准大小") target:self action:@selector(resetWindowScale:)];
    resetSize.bezelStyle = NSBezelStyleRounded;
    resetSize.enabled = !self.positionLocked;
    NSArray<NSView *> *behaviorControls = @[
        [self settingsCheckbox:HUDL(@"始终置顶") action:@selector(toggleAlwaysOnTop:) state:self.alwaysOnTop],
        [self settingsCheckbox:HUDL(@"锁定位置和大小") action:@selector(togglePositionLock:) state:self.positionLocked],
        [self settingsCheckbox:HUDL(@"显示详细信息") action:@selector(toggleDetail:) state:!self.compact],
        [self settingsCheckbox:HUDL(@"保存每分钟趋势") action:@selector(toggleHistory:) state:self.historyEnabled],
        [self settingsCheckbox:HUDL(@"登录后自动启动") action:@selector(toggleLaunchAtLogin:) state:[self launchAtLoginEnabled]]
    ];
    ...
    NSBox *behaviorBox = [self settingsGroup:HUDL(@"常驻与显示") controls:behaviorControls];
    NSBox *sizeBox = [self settingsGroup:HUDL(@"外观") controls:@[sizeStack]];
    ...
    self.updateStatusLabel = [NSTextField wrappingLabelWithString:[NSString stringWithFormat:HUDL(@"当前版本 %@ · 每天自动检查一次"), version]];
    self.updateStatusLabel.font = [NSFont systemFontOfSize:12.5 weight:NSFontWeightRegular];
    self.updateButton = [NSButton buttonWithTitle:HUDL(@"检查更新") target:self action:@selector(checkForUpdatesManually:)];
    ...
    NSBox *updateBox = [self settingsGroup:HUDL(@"软件更新") controls:@[updateStack]];
    ...
    NSTextField *sourceText = [NSTextField wrappingLabelWithString:HUDL(@"电脑性能：macOS系统接口。  Codex额度、订阅类型、账户Token与任务历史：本机Codex官方接口。  订阅日期：用户手动填写并仅保存在本机。  5小时/24小时/每周Token与费用：只读取本机会话记录中的时间、模型和Token计数，属于本地统计。  OpenAI服务状态：官方公开状态页，不带账号信息且默认关闭。  任务活动：只读检查近期写入记录。  更新：仅访问本项目GitHub Release。")];
    sourceText.font = [NSFont systemFontOfSize:11.5]; sourceText.textColor = NSColor.labelColor;
    NSTextField *permissionText = [NSTextField wrappingLabelWithString:HUDL(@"默认不需要屏幕录制、辅助功能、完全磁盘访问、浏览器Cookie或API密钥；费用缓存不保存提示词、回复或工具内容；单任务实时Token速度因无法连接桌面版同一接口实例，当前不会显示。")];
    permissionText.font = [NSFont systemFontOfSize:11.5]; permissionText.textColor = NSColor.secondaryLabelColor;
    NSBox *dataBox = [self settingsGroup:HUDL(@"数据来源与权限") controls:@[sourceText, permissionText]];

    NSTextField *hint = [NSTextField labelWithString:HUDL(@"所有勾选状态集中显示；主页扩展和长期历史默认关闭，按需勾选。")];
    ...
    [language addItemsWithTitles:@[@"简体中文", @"繁體中文", @"English", @"日本語", @"한국어"]];
    NSArray *languageCodes = @[@"zh-Hans", @"zh-Hant", @"en", @"ja", @"ko"];
    NSString *selectedLanguage = [NSUserDefaults.standardUserDefaults stringForKey:@"displayLanguage"] ?: HUDLanguage();
    NSUInteger languageIndex = [languageCodes indexOfObject:selectedLanguage];
    [language selectItemAtIndex:languageIndex == NSNotFound ? 0 : (NSInteger)languageIndex];
    language.target = self; language.action = @selector(changeDisplayLanguage:);
    NSPopUpButton *currency = [[NSPopUpButton alloc] initWithFrame:NSZeroRect pullsDown:NO];
    NSLocale *displayLocale = [NSLocale localeWithLocaleIdentifier:HUDLanguage()];
    for (NSString *code in @[@"CNY", @"USD", @"EUR", @"JPY", @"KRW"]) {
        [currency addItemWithTitle:[NSString stringWithFormat:@"%@ · %@", code, [displayLocale localizedStringForCurrencyCode:code] ?: code]];
    }
    [currency selectItemAtIndex:[@[@"CNY", @"USD", @"EUR", @"JPY", @"KRW"] indexOfObject:HUDCurrency()]];
    currency.target = self; currency.action = @selector(changeDisplayCurrency:);
    NSTextField *moneyNote = [NSTextField wrappingLabelWithString:HUDL(@"金额是按参考API标准价格折算的估值，不是订阅实际账单；未包含Fast等加价。子任务继承量不重复计入，无法可靠计价的记录不猜价格。")];
    moneyNote.font = [NSFont systemFontOfSize:11.5];
    NSTextField *ratesNote = [NSTextField wrappingLabelWithString:[NSString stringWithFormat:HUDL(@"参考价格：2026-09-11 · 汇率日期：%@ · 每天最多更新一次，失败保留上次汇率"), HUDExchangeRateDate()]];
    ratesNote.font = [NSFont systemFontOfSize:11.5];
    NSBox *languageBox = [self settingsGroup:HUDL(@"语言与币种") controls:@[
        [NSTextField labelWithString:HUDL(@"界面语言（重新打开悬浮窗后生效）")], language,
        [NSTextField labelWithString:HUDL(@"显示币种（立即生效）")], currency, moneyNote, ratesNote]];
    [NSLayoutConstraint activateConstraints:@[
...
- (void)changeDisplayLanguage:(NSPopUpButton *)sender {
    NSArray *codes = @[@"zh-Hans", @"zh-Hant", @"en", @"ja", @"ko"];
    if (sender.indexOfSelectedItem >= 0 && sender.indexOfSelectedItem < (NSInteger)codes.count)
        [NSUserDefaults.standardUserDefaults setObject:codes[sender.indexOfSelectedItem] forKey:@"displayLanguage"];
}

- (void)changeDisplayCurrency:(NSPopUpButton *)sender {
    NSArray *codes = @[@"CNY", @"USD", @"EUR", @"JPY", @"KRW"];
    if (sender.indexOfSelectedItem >= 0 && sender.indexOfSelectedItem < (NSInteger)codes.count)
        [NSUserDefaults.standardUserDefaults setObject:codes[sender.indexOfSelectedItem] forKey:@"displayCurrency"];
    [self updateCodexDisplay];
}
...
- (NSMenu *)settingsMenu {
    ...
    NSMenuItem *allSettings = [[NSMenuItem alloc] initWithTitle:HUDL(@"显示设置…") action:@selector(showSettingsWindow:) keyEquivalent:@""];
    allSettings.target = self; [menu addItem:allSettings]; [menu addItem:NSMenuItem.separatorItem];
    NSMenu *page = [NSMenu new];
    NSArray<NSString *> *pageNames = @[HUDL(@"主页"), @"Codex", HUDL(@"电脑性能")];
    ...
    NSMenuItem *pageRoot = [[NSMenuItem alloc] initWithTitle:HUDL(@"显示页面") action:nil keyEquivalent:@""]; pageRoot.submenu = page; [menu addItem:pageRoot];
    NSMenu *homeModules = [NSMenu new];
    [homeModules addItem:[self item:HUDL(@"当前任务活动（本机推测）") action:@selector(toggleHomeTaskActivity:) state:self.homeShowTaskActivity]];
    [homeModules addItem:[self item:HUDL(@"最近任务（历史）") action:@selector(toggleHomeRecentTasks:) state:self.homeShowRecentTasks]];
    [homeModules addItem:[self item:HUDL(@"5小时额度") action:@selector(toggleHomeFiveHour:) state:self.homeShowFiveHour]];
    [homeModules addItem:[self item:HUDL(@"每周额度") action:@selector(toggleHomeWeekly:) state:self.homeShowWeekly]];
    [homeModules addItem:[self item:HUDL(@"订阅类型") action:@selector(toggleHomePlan:) state:self.homeShowPlan]];
    [homeModules addItem:[self item:HUDL(@"账户Token统计") action:@selector(toggleHomeUsage:) state:self.homeShowUsage]];
    [homeModules addItem:[self item:HUDL(@"5小时/24小时/每周Token对比") action:@selector(toggleHomeTokenWindows:) state:self.homeShowTokenWindows]];
    [homeModules addItem:[self item:HUDL(@"Token用量与费用") action:@selector(toggleHomeLocalCost:) state:self.homeShowLocalCost]];
    [homeModules addItem:[self item:HUDL(@"额度趋势预测") action:@selector(toggleHomeQuotaForecast:) state:self.homeShowQuotaForecast]];
    [homeModules addItem:[self item:HUDL(@"OpenAI服务状态") action:@selector(toggleHomeServiceStatus:) state:self.homeShowServiceStatus]];
    [homeModules addItem:[self item:HUDL(@"完整额度列表与恢复次数") action:@selector(toggleHomeQuotaDetails:) state:self.homeShowQuotaDetails]];
    [homeModules addItem:[self item:HUDL(@"模型专属额度") action:@selector(toggleHomeModelQuota:) state:self.homeShowModelQuota]];
    [homeModules addItem:[self item:HUDL(@"最长单次任务时长") action:@selector(toggleHomeLongestTurn:) state:self.homeShowLongestTurn]];
    [homeModules addItem:[self item:HUDL(@"历史最长连续天数") action:@selector(toggleHomeLongestStreak:) state:self.homeShowLongestStreak]];
    [homeModules addItem:[self item:HUDL(@"历史单日峰值Token") action:@selector(toggleHomePeakDailyTokens:) state:self.homeShowPeakDailyTokens]];
    [homeModules addItem:NSMenuItem.separatorItem];
    [homeModules addItem:[self item:HUDL(@"瓶颈判断") action:@selector(toggleHomeDiagnosis:) state:self.homeShowDiagnosis]];
    [homeModules addItem:[self item:HUDL(@"电脑核心状态") action:@selector(toggleHomeSystem:) state:self.homeShowSystem]];
    [homeModules addItem:[self item:HUDL(@"Codex性能占用") action:@selector(toggleHomeAttribution:) state:self.homeShowAttribution]];
    [homeModules addItem:[self item:HUDL(@"内存占用排行") action:@selector(toggleHomeMemoryApps:) state:self.homeShowMemoryApps]];
    [homeModules addItem:[self item:HUDL(@"CPU趋势") action:@selector(toggleHomeTrend:) state:self.homeShowTrend]];
    NSMenuItem *homeRoot = [[NSMenuItem alloc] initWithTitle:HUDL(@"主页内容") action:nil keyEquivalent:@""]; homeRoot.submenu = homeModules; [menu addItem:homeRoot];
    NSMenu *modules = [NSMenu new];
    [modules addItem:[self item:HUDL(@"当前任务活动（本机推测）") action:@selector(toggleTaskActivity:) state:self.showTaskActivity]];
    [modules addItem:[self item:HUDL(@"最近任务（历史）") action:@selector(toggleRecentTasks:) state:self.showRecentTasks]];
    [modules addItem:[self item:HUDL(@"5小时额度") action:@selector(toggleFiveHourQuota:) state:self.showFiveHourQuota]];
    [modules addItem:[self item:HUDL(@"每周额度") action:@selector(toggleWeeklyQuota:) state:self.showWeeklyQuota]];
    [modules addItem:[self item:HUDL(@"订阅类型") action:@selector(togglePlan:) state:self.showPlan]];
    [modules addItem:[self item:HUDL(@"账户Token统计") action:@selector(toggleUsage:) state:self.showUsage]];
    [modules addItem:[self item:HUDL(@"5小时/24小时/每周Token对比") action:@selector(toggleTokenWindows:) state:self.showTokenWindows]];
    [modules addItem:[self item:HUDL(@"Token用量与费用") action:@selector(toggleLocalCost:) state:self.showLocalCost]];
    [modules addItem:[self item:HUDL(@"额度趋势预测") action:@selector(toggleQuotaForecast:) state:self.showQuotaForecast]];
    [modules addItem:[self item:HUDL(@"OpenAI服务状态") action:@selector(toggleServiceStatus:) state:self.showServiceStatus]];
    [modules addItem:[self item:HUDL(@"完整额度列表与恢复次数") action:@selector(toggleQuotaDetails:) state:self.showQuotaDetails]];
    [modules addItem:[self item:HUDL(@"最长单次任务时长") action:@selector(toggleLongestTurn:) state:self.showLongestTurn]];
    [modules addItem:[self item:HUDL(@"历史最长连续天数") action:@selector(toggleLongestStreak:) state:self.showLongestStreak]];
    [modules addItem:[self item:HUDL(@"历史单日峰值Token") action:@selector(togglePeakDailyTokens:) state:self.showPeakDailyTokens]];
    [modules addItem:[self item:HUDL(@"电脑核心状态") action:@selector(toggleSystem:) state:self.showSystem]];
    [modules addItem:[self item:HUDL(@"Codex性能占用") action:@selector(toggleAttribution:) state:self.showAttribution]];
    [modules addItem:[self item:HUDL(@"10分钟CPU趋势") action:@selector(toggleTrend:) state:self.showTrend]];
    [modules addItem:[self item:HUDL(@"内存占用排行") action:@selector(toggleMemoryApps:) state:self.showMemoryApps]];
    NSMenuItem *moduleRoot = [[NSMenuItem alloc] initWithTitle:HUDL(@"分页面内容") action:nil keyEquivalent:@""]; moduleRoot.submenu = modules; [menu addItem:moduleRoot];
    NSMenu *advanced = [NSMenu new];
    [advanced addItem:[self item:HUDL(@"模型专属额度（有数据时显示）") action:@selector(toggleModelQuota:) state:self.showModelQuota]];
    NSMenuItem *advancedRoot = [[NSMenuItem alloc] initWithTitle:HUDL(@"高级显示") action:nil keyEquivalent:@""]; advancedRoot.submenu = advanced; [menu addItem:advancedRoot];
    ...
    NSMenuItem *opacityRoot = [[NSMenuItem alloc] initWithTitle:HUDL(@"透明度") action:nil keyEquivalent:@""]; opacityRoot.submenu = opacity; [menu addItem:opacityRoot];
    NSMenuItem *resetScale = [self item:HUDL(@"恢复标准大小") action:@selector(resetWindowScale:) state:NO]; resetScale.enabled = !self.positionLocked; [menu addItem:resetScale];
    NSMenu *colors = [NSMenu new];
    for (NSArray *pair in @[@[HUDL(@"绿色"), @"green"], @[HUDL(@"蓝色"), @"blue"], @[HUDL(@"紫色"), @"purple"], @[HUDL(@"橙色"), @"orange"]]) { NSMenuItem *i = [self item:pair[0] action:@selector(setAccent:) state:[self.accentName isEqualToString:pair[1]]]; i.representedObject = pair[1]; [colors addItem:i]; }
    NSMenuItem *colorRoot = [[NSMenuItem alloc] initWithTitle:HUDL(@"强调颜色") action:nil keyEquivalent:@""]; colorRoot.submenu = colors; [menu addItem:colorRoot];
    NSMenu *refresh = [NSMenu new];
    for (NSNumber *value in @[@5, @10, @20]) { NSMenuItem *i = [self item:[NSString stringWithFormat:HUDL(@"%@秒"), value] action:@selector(setRefresh:) state:fabs(self.refreshInterval - value.doubleValue) < 0.1]; i.tag = value.integerValue; [refresh addItem:i]; }
    NSMenuItem *refreshRoot = [[NSMenuItem alloc] initWithTitle:HUDL(@"电脑数据刷新") action:nil keyEquivalent:@""]; refreshRoot.submenu = refresh; [menu addItem:refreshRoot];
    NSMenu *positions = [NSMenu new];
    for (NSArray *pair in @[@[HUDL(@"左下角"), @"bottomLeft"], @[HUDL(@"左上角"), @"topLeft"], @[HUDL(@"右下角"), @"bottomRight"], @[HUDL(@"右上角"), @"topRight"]]) { NSMenuItem *i = [self item:pair[0] action:@selector(moveCornerFromMenu:) state:NO]; i.representedObject = pair[1]; i.enabled = !self.positionLocked; [positions addItem:i]; }
    NSMenuItem *positionRoot = [[NSMenuItem alloc] initWithTitle:HUDL(@"屏幕位置") action:nil keyEquivalent:@""]; positionRoot.submenu = positions; [menu addItem:positionRoot];
    [menu addItem:[self item:HUDL(@"始终置顶") action:@selector(toggleAlwaysOnTop:) state:self.alwaysOnTop]];
    [menu addItem:[self item:HUDL(@"锁定位置和大小") action:@selector(togglePositionLock:) state:self.positionLocked]];
    [menu addItem:[self item:HUDL(@"最小化到程序栏") action:@selector(minimizeToDock:) state:NO]];
    [menu addItem:[self item:(self.collapsed ? HUDL(@"退出窄栏模式") : HUDL(@"窄栏模式")) action:@selector(toggleCollapsed:) state:self.collapsed]];
    [menu addItem:[self item:HUDL(@"显示详细信息") action:@selector(toggleDetail:) state:!self.compact]];
    [menu addItem:[self item:HUDL(@"保存每分钟趋势") action:@selector(toggleHistory:) state:self.historyEnabled]];
    [menu addItem:[self item:HUDL(@"登录后自动启动") action:@selector(toggleLaunchAtLogin:) state:[self launchAtLoginEnabled]]];
    [menu addItem:[self item:HUDL(@"检查更新…") action:@selector(checkForUpdatesManually:) state:NO]];
    [menu addItem:NSMenuItem.separatorItem];
    NSMenuItem *open = [[NSMenuItem alloc] initWithTitle:HUDL(@"打开趋势数据文件夹") action:@selector(openDataFolder:) keyEquivalent:@""]; open.target = self; [menu addItem:open];
    NSMenuItem *quit = [[NSMenuItem alloc] initWithTitle:HUDL(@"退出悬浮窗") action:@selector(quitHUD:) keyEquivalent:@""]; quit.target = self; [menu addItem:quit];
    return menu;
}
...
static void CountSettingsControls(NSView *view, NSInteger *checkboxCount, NSInteger *hiddenFiveHourCount, NSInteger *hiddenPlanCount, NSInteger *optionalHistoryOffCount, NSInteger *resetButtonCount) {
    if ([view isKindOfClass:NSButton.class] && ![view isKindOfClass:NSPopUpButton.class]) {
        ...
        if ([button.title isEqualToString:HUDL(@"恢复标准大小")]) (*resetButtonCount)++;
            ...
            if (([button.title isEqualToString:HUDL(@"最长单次任务时长")] || [button.title isEqualToString:HUDL(@"历史最长连续天数")] || [button.title isEqualToString:HUDL(@"历史单日峰值Token")]) && button.state == NSControlStateValueOff) (*optionalHistoryOffCount)++;
        }
    }
...
static int RunUIDiagnostic(void) {
    ...
    delegate.subscriptionDateString = @"2099-09-24";
    ...
    BOOL taskCenterEntryPass = taskCenterRequested && taskCenterButtonLayoutPass && [delegate.hudView.taskCenterButton.toolTip isEqualToString:HUDL(@"打开任务中心")] && delegate.hudView.taskCenterButton.image != nil;
    ...
    longTextHUD.usageCard.subtitleLabel.stringValue = HUDL(@"今日数据未返回 · 7天 7900953979 · 上升125% · 不等于额度");
    ...
    provider.snapshot.recentTasksErrorText = HUDL(@"当前未返回"); refreshPolicyPass = refreshPolicyPass && [delegate codexAccountRefreshInterval] == 300.0;
    provider.snapshot.interfaceFailureKinds = @{@"5": @"network"}; refreshPolicyPass = refreshPolicyPass && [delegate codexAccountRefreshInterval] == 60.0;
    provider.snapshot.interfaceFailureKinds = @{}; provider.snapshot.recentTasksErrorText = nil;
    ...
    BOOL stateFallbackPass = [delegate.hudView.weeklyCard.windowLabel.stringValue isEqualToString:HUDL(@"上次数据")] && [delegate.hudView.usageCard.valueLabel.stringValue containsString:@"8/5"] && ![delegate.hudView.usageCard.valueLabel.stringValue containsString:HUDL(@"待结算")];
    BOOL subscriptionCardPass = [delegate.hudView.planCard.titleLabel.stringValue isEqualToString:HUDL(@"订阅 · 截止/续费")] &&
                                [delegate.hudView.planCard.valueLabel.stringValue isEqualToString:@"Pro · 9/24"] &&
                                [delegate.hudView.planCard.subtitleLabel.stringValue containsString:[NSString stringWithFormat:HUDL(@"%@ · 手动设置"), @""]] &&
                                [delegate.hudView.homePlanCard.valueLabel.stringValue isEqualToString:delegate.hudView.planCard.valueLabel.stringValue];
    ...
    provider.snapshot.activityAvailable = NO; provider.snapshot.activityErrorText = HUDL(@"近期会话记录已迁移、压缩或暂不可读，无法可靠判断活动");
    ...
    BOOL migrationPresentationPass = [delegate.hudView.taskActivityCard.valueLabel.stringValue isEqualToString:HUDL(@"当前无法可靠判断")] && [activityStatus isEqualToString:HUDL(@"任务活动暂时无法可靠判断")];
    provider.snapshot.activityAvailable = YES; provider.snapshot.activityErrorText = nil; provider.snapshot.activityPartial = YES; provider.snapshot.activityNoteText = HUDL(@"部分近期会话记录已迁移、压缩或暂不可读"); provider.snapshot.activeTaskCount = 0;
    [delegate updateCodexDisplay];
    migrationPresentationPass = migrationPresentationPass && [delegate.hudView.taskActivityCard.valueLabel.stringValue isEqualToString:HUDL(@"暂无可确认的活跃任务")] && [delegate.hudView.taskActivityCard.subtitleLabel.stringValue containsString:provider.snapshot.activityNoteText];
    ...
    BOOL hiddenContentPass = [details containsString:HUDL(@"每周")] && ![details containsString:HUDL(@"5小时")] && ![details containsString:HUDL(@"订阅")] && [status isEqualToString:HUDL(@"Codex数据正常")];
    ...
    BOOL allowancePass = [allowanceVisible isEqualToString:HUDL(@"官方：普通包含用量暂不可用")] && ![allowanceHidden isEqualToString:HUDL(@"官方：普通包含用量暂不可用")];
    provider.snapshot.ordinaryUsageUpdatedAt = 1;
    allowancePass = allowancePass && ![[delegate visibleCodexStatus:provider.snapshot fiveHour:NO weekly:YES plan:NO usage:NO model:NO activity:NO recent:NO] containsString:HUDL(@"普通包含用量")];
    ...
    printf("subscription_date_settings_test=%s\n", subscriptionSettingsPass ? "pass" : "fail");
    ...
    printf("subscription_date_card_test=%s\n", subscriptionCardPass ? "pass" : "fail");
    ...
    return settingsPass && orderPass && taskCenterEntryPass && homeLayoutPass && longTextLayoutPass && refreshPolicyPass && stateFallbackPass && subscriptionCardPass && migrationPresentationPass && scalePass && positionLockPass && hiddenSamplingPass && hiddenServiceStatusPass && timerTolerancePass && cardVisibilityPass && hiddenContentPass && allowancePass ? 0 : 5;
}
...
static int RunUpdateHandoffDiagnostic(void) {
    // The old diagnostic restarted the user's actual LaunchAgent even for temporary app copies.
    // Failure-path verification now lives in isolated fixtures and is enforced by CI.
    fprintf(stderr, "update_launch_agent_integration_test=not_run\n");
    fprintf(stderr, "请运行 sh tests/test-maintenance.sh；旧实机交接测试已停用，避免重启正在使用的HUD。\n");
    return 7;
}
...
static int RunLogicDiagnostic(void) {
    ...
    NSDate *subscriptionNow = HUDSubscriptionDateFromString(@"2030-01-10");
    NSDictionary *futureSubscription = HUDSubscriptionDatePresentation(@"2030-01-29", subscriptionNow);
    NSDictionary *todaySubscription = HUDSubscriptionDatePresentation(@"2030-01-10", subscriptionNow);
    NSDictionary *pastSubscription = HUDSubscriptionDatePresentation(@"2030-01-09", subscriptionNow);
    BOOL subscriptionDatePass = [HUDSubscriptionDateStringFromDate(HUDSubscriptionDateFromString(@"2030-01-29")) isEqualToString:@"2030-01-29"] &&
                                [futureSubscription[@"fullDate"] isEqualToString:@"2030/01/29"] && [futureSubscription[@"shortDate"] isEqualToString:@"1/29"] && [futureSubscription[@"days"] integerValue] == 19 && [futureSubscription[@"relative"] isEqualToString:@"还剩19天"] &&
    ...
    BOOL costPricingPass = [shortCost[@"available"] boolValue] && fabs([shortCost[@"cost"] doubleValue] - 0.284) < 0.000001 && [longCost[@"longContext"] boolValue] && fabs([longCost[@"cost"] doubleValue] - 1.98) < 0.000001 && ![unknownCost[@"available"] boolValue];
...
static int RunUISnapshot(void) {
    ...
    NSDate *snapshotSubscriptionDate = [NSCalendar.currentCalendar dateByAddingUnit:NSCalendarUnitDay value:30 toDate:NSDate.date options:0];
    delegate.subscriptionDateString = HUDSubscriptionDateStringFromDate(snapshotSubscriptionDate);
    ...
    NSScrollView *settingsScroll = [view isKindOfClass:NSScrollView.class] ? (NSScrollView *)view : nil;
    BOOL subscriptionWritten = NO;
    NSString *subscriptionPath = @"/private/tmp/codex-monitor-hud-subscription-settings.png";
    if (settingsScroll.documentView && delegate.subscriptionDateStatusLabel) {
        NSRect targetRect = [delegate.subscriptionDateStatusLabel convertRect:delegate.subscriptionDateStatusLabel.bounds toView:settingsScroll.documentView];
        [settingsScroll.documentView scrollRectToVisible:NSInsetRect(targetRect, -20, -140)];
        [settingsScroll layoutSubtreeIfNeeded];
        NSBitmapImageRep *subscriptionBitmap = [settingsScroll bitmapImageRepForCachingDisplayInRect:settingsScroll.bounds];
        [settingsScroll cacheDisplayInRect:settingsScroll.bounds toBitmapImageRep:subscriptionBitmap];
        NSData *subscriptionPNG = [subscriptionBitmap representationUsingType:NSBitmapImageFileTypePNG properties:@{}];
        subscriptionWritten = [subscriptionPNG writeToFile:subscriptionPath atomically:YES];
    }
    ...
    NSDictionary<NSString *, id> *snapshotSubscription = HUDSubscriptionDatePresentation(delegate.subscriptionDateString, NSDate.date);
    hud.planCard.titleLabel.stringValue = @"订阅 · 截止/续费";
    hud.planCard.valueLabel.stringValue = [NSString stringWithFormat:@"Pro · %@", snapshotSubscription[@"shortDate"]];
    hud.planCard.subtitleLabel.stringValue = [NSString stringWithFormat:@"%@ · 手动设置", snapshotSubscription[@"relative"]];
    ...
    printf("subscription_settings_snapshot=%s\n", subscriptionWritten ? subscriptionPath.UTF8String : "failed");
    printf("hud_snapshot=%s\n", hudWritten ? hudPath.UTF8String : "failed");
    return written && subscriptionWritten && hudWritten ? 0 : 8;
}
...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        if (argc == 3 && strcmp(argv[1], "--update-health-probe") == 0) return HUDUpdateProcessProbe([NSString stringWithUTF8String:argv[2]], NO);
        if (argc == 3 && strcmp(argv[1], "--update-stop-probe") == 0) return HUDUpdateProcessProbe([NSString stringWithUTF8String:argv[2]], YES);
